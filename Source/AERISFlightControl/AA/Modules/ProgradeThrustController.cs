...
namespace AtmosphereAutopilot
{
    ...
    public sealed class ProgradeThrustController : SISOController
    {
        ...
        // Reuses AA's native speed PID for propulsion extensions. This only produces a
        // high-level demand; extensions such as APP remain responsible for execution.
        public float ComputeExternalPidDemand(float target_value)
        {
            current_v = vessel == null ? 0.0 : vessel.srfSpeed;
            desired_v = target_value;
            if (pid.KI != 0.0)
            {
                if (pid.KP != 0.0) pid.IntegralClamp = 1.0 / pid.KP;
                pid.AccumulatorClamp = 1.0 / pid.KI;
            }
            return Mathf.Clamp01((float)pid.Control(current_v, desired_v, TimeWarp.fixedDeltaTime));
        }
        ...
        public override float ApplyControl(FlightCtrlState cntrl, float target_value)
        {
            ...
            else
            {
                ...
                // Predictive approach damping: for conventional engines, begin removing thrust
                // before the measured speed reaches target when current acceleration would carry
                // the vessel through the setpoint. This uses AA's existing FlightModel acceleration.
                double predicted_v = current_v + Math.Max(0.0, current_acc) * 1.50;
                v_error = desired_v - current_v;
                double predictive_error = desired_v - predicted_v;
                desired_acc = Kp_v * predictive_error;
                // Above target, force a positive thrust unwind rather than waiting for model lag.
                if (current_v > desired_v)
                    desired_acc = Math.Min(desired_acc, -0.35 * (current_v - desired_v));
