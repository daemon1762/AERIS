...
namespace AtmosphereAutopilot
{
    ...
    public abstract class AngularVelAdaptiveController : AutopilotModule
    {
        ...
        // v0.8.31: expose the moderation boundary as read-only telemetry.  This makes
        // external AERIS rate ownership auditable without changing AA's final-writer role.
        public bool ExternalRateControlActive { get; private set; }
        public bool DesiredAngularVelocityModerated { get; private set; }
        public float RequestedDesiredAngularVelocity { get; private set; }
        public float AppliedDesiredAngularVelocity { get; private set; }
        ...
        void ResetDesiredVelocityTelemetry()
        {
            ExternalRateControlActive = false;
            DesiredAngularVelocityModerated = false;
            RequestedDesiredAngularVelocity = 0f;
            AppliedDesiredAngularVelocity = 0f;
        }
        ...
        public float ApplyControl(FlightCtrlState cntrl, float target_value, float target_acc = 0.0f)
        {
            ...
            ExternalRateControlActive = !user_controlled;
            RequestedDesiredAngularVelocity = desired_v;
            desired_v = process_desired_v(desired_v, user_controlled);      // moderation stage
            AppliedDesiredAngularVelocity = desired_v;
            DesiredAngularVelocityModerated =
                Math.Abs(AppliedDesiredAngularVelocity - RequestedDesiredAngularVelocity) > 1e-6f;
    ...
    public abstract class PitchYawAngularVelocityController : AngularVelAdaptiveController
    {
        ...
        // Current AA moderation envelope after AoA/G/model constraints. Values are
        // radians per second, matching the native angular-velocity controller.
        public bool ModerationEnvelopeAvailable { get; private set; }
        public float ModerationLowerAngularVelocity { get; private set; }
        public float ModerationUpperAngularVelocity { get; private set; }
        ...
        protected override float process_desired_v(float des_v, bool user_input)
        {
            ...
            ModerationEnvelopeAvailable = false;
            ModerationLowerAngularVelocity = -max_v_construction;
            ModerationUpperAngularVelocity = max_v_construction;
            ...
            // desired_v moderation section.  Build the AA AoA/G rate envelope for
            // both pilot and external-rate ownership.  Historically it was applied
            // only inside the user-controlled branch, so a direct AERIS pitch-rate
            // request bypassed these otherwise-active safety limits.
            ...
            if (float.IsNaN(min_v) || float.IsInfinity(min_v) ||
                float.IsNaN(max_v) || float.IsInfinity(max_v) || min_v > max_v)
            {
                // A transiently singular identification model must never inject NaN
                // or an inverted clamp into either the pilot or external rate path.
                moderated = false;
                max_v = float.IsNaN(max_v_construction) || float.IsInfinity(max_v_construction)
                    ? 0.7f : Mathf.Abs(max_v_construction);
                min_v = -max_v;
            }

            ModerationEnvelopeAvailable = moderated;
            ModerationLowerAngularVelocity = min_v;
            ModerationUpperAngularVelocity = max_v;
            ...
            else if (moderated && axis == PITCH)
            {
                // External AERIS pitch-rate commands already have native units and must
                // not be reinterpreted as stick position. Clamp only their outward demand.
                // Yaw transport is deliberately unchanged in this release.
                des_v = Mathf.Clamp(des_v, min_v, max_v);
            }
            return des_v;
        }
