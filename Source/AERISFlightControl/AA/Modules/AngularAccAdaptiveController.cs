...
namespace AtmosphereAutopilot
{
    ...
    public abstract class AngularAccAdaptiveController : SISOController
    {
        ...
        // Telemetry writers
        protected AERISAsyncFileChannel controlWriter, v_writer, acc_writer, prediction_writer,
        ...
        public override float ApplyControl(FlightCtrlState cntrl, float target_value)
        {
            ...
            if (write_telemetry)
            {
                desire_acc_writer.WriteGeneralNumber(target_value, true);
                acc_writer.WriteGeneralNumber(acc, true);
                v_writer.WriteGeneralNumber(imodel.AngularVel(axis), true);
                //prediction_writer.WriteGeneralNumber(target_value, true);
                aoa_writer.WriteGeneralNumber(imodel.AoA(axis), true);
                airspd_writer.WriteGeneralNumber((imodel.up_srf_v + imodel.fwd_srf_v).magnitude, true);
                density_writer.WriteGeneralNumber(vessel.atmDensity, true);
            }
            ...
            // AA must remain the normal FlightCtrlState writer. v0.4.42 accidentally
            // omitted this write during the Quiet Mode cleanup, leaving FBW active
            // but unable to apply its calculated axis output.
            ControlUtils.set_raw_output(cntrl, axis, output);

            if (write_telemetry)
            {
                controlWriter.WriteGeneralNumber(csurf_output, true);
                outputWriter.WriteGeneralNumber(output, true);
            }
        ...
        void create_writers()
        {
            string root = Path.Combine(KSPUtil.ApplicationRootPath, "Resources");
            controlWriter = OpenTelemetry(root, "control.csv");
            v_writer = OpenTelemetry(root, "v.csv");
            acc_writer = OpenTelemetry(root, "acc.csv");
            desire_acc_writer = OpenTelemetry(root, "desire.csv");
            prediction_writer = OpenTelemetry(root, "predict.csv");
            aoa_writer = OpenTelemetry(root, "aoa.csv");
            airspd_writer = OpenTelemetry(root, "airspd.csv");
            density_writer = OpenTelemetry(root, "density.csv");
            outputWriter = OpenTelemetry(root, "output.csv");
        }

        void close_writers()
        {
            CloseTelemetry(ref controlWriter); CloseTelemetry(ref v_writer);
            CloseTelemetry(ref acc_writer); CloseTelemetry(ref desire_acc_writer);
            CloseTelemetry(ref prediction_writer); CloseTelemetry(ref aoa_writer);
            CloseTelemetry(ref airspd_writer); CloseTelemetry(ref density_writer);
            CloseTelemetry(ref outputWriter);
        }

        static AERISAsyncFileChannel OpenTelemetry(string root, string name)
        {
            return new AERISAsyncFileChannel(Path.Combine(root, name), false,
                AERISFileRecordPriority.Verbose);
        }

        static void CloseTelemetry(ref AERISAsyncFileChannel channel)
        {
            AERISAsyncFileChannel closing = channel;
            channel = null;
    ...
    public abstract class PitchYawAngularAccController : AngularAccAdaptiveController
    {
        ...
        protected override float get_required_input(FlightCtrlState cntrl, float target_value)
        {
            ...
            if (write_telemetry)
            {
                prediction_writer.WriteGeneralNumber(model_predicted_acc, true);
            }
