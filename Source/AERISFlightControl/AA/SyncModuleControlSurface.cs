...
namespace AtmosphereAutopilot
{
    ...
    public class SyncModuleControlSurface: ModuleControlSurface
    {
        ...
        private const BindingFlags LifecycleBindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
        private static bool lifecycleCleanupActivationLogged;
        private static bool lifecycleCleanupWarningLogged;

        public override void OnStart(PartModule.StartState state)
        {
            // ModuleControlSurface registers private GameEvents callbacks from OnStart.
            // Remove any callbacks left by a prior lifecycle before the stock module
            // registers its fresh set. This is idempotent on a first start.
            CleanupStockControlSurfaceCallbacks("pre-start");
            base.OnStart(state);
        ...
        /// <summary>
        /// Unity message intentionally redeclared because ModuleControlSurface.OnDestroy
        /// is public but non-virtual. The stock cleanup is still invoked exactly once,
        /// with explicit callback removal before and after it so subclasses cannot leave
        /// private stock handlers in the global GameEvents tables.
        /// </summary>
        public new void OnDestroy()
        {
            bool preCleanupOk = CleanupStockControlSurfaceCallbacks("pre-destroy");
            try
            {
                base.OnDestroy();
            }
            ...
            {
                bool postCleanupOk = CleanupStockControlSurfaceCallbacks("post-destroy");
                if (preCleanupOk && postCleanupOk && !lifecycleCleanupActivationLogged)
                {
                    lifecycleCleanupActivationLogged = true;
                    AERISLogger.Info("[AA/CONTROL_SURFACE_LIFECYCLE] explicit stock callback cleanup active.");
                }
            }
        }

        private bool CleanupStockControlSurfaceCallbacks(string stage)
        {
            bool success = true;
            success &= RemoveStockGameEventCallback(GameEvents.onEditorPartEvent, "OnEditorPartEvent", stage);
            success &= RemoveStockGameEventCallback(GameEvents.onPartActionUIShown, "OnPartActionUIShown", stage);
            success &= RemoveStockGameEventCallback(GameEvents.onPartActionUIDismiss, "OnPartActionUIDismiss", stage);
            success &= RemoveStockGameEventCallback(GameEvents.onVariantApplied, "onVariantApplied", stage);
            success &= RemoveStockGameEventCallback(GameEvents.onVesselReferenceTransformSwitch, "onVesselReferenceTransformSwitch", stage);
            return success;
        }

        private bool RemoveStockGameEventCallback(object eventSource, string handlerName, string stage)
        {
            if (eventSource == null)
                return true;
            ...
            try
            {
                MethodInfo handler = FindInstanceMethod(typeof(ModuleControlSurface), handlerName);
                MethodInfo remove = FindSingleDelegateParameterMethod(eventSource.GetType(), "Remove");
                if (handler == null || remove == null)
                {
                    LogLifecycleCleanupWarningOnce(stage, handlerName, "handler/remove method unavailable");
                    return false;
                }

                Type delegateType = remove.GetParameters()[0].ParameterType;
                Delegate callback = Delegate.CreateDelegate(delegateType, this, handler, false);
                if (callback == null)
                {
                    LogLifecycleCleanupWarningOnce(stage, handlerName, "delegate bind failed");
                    return false;
                }

                remove.Invoke(eventSource, new object[] { callback });
                return true;
            }
            catch (Exception ex)
            {
                LogLifecycleCleanupWarningOnce(stage, handlerName, ex.GetType().Name + ": " + ex.Message);
                return false;
            }
        }
        ...
        private static MethodInfo FindInstanceMethod(Type type, string methodName)
        {
            ...
            MethodInfo[] methods = type.GetMethods(LifecycleBindingFlags);
        ...
        private static MethodInfo FindSingleDelegateParameterMethod(Type type, string methodName)
        {
            if (type == null || string.IsNullOrEmpty(methodName))
                return null;
        ...
        private static void LogLifecycleCleanupWarningOnce(string stage, string handlerName, string detail)
        {
            if (lifecycleCleanupWarningLogged)
                return;
            lifecycleCleanupWarningLogged = true;
            AERISLogger.Warn("[AA/CONTROL_SURFACE_LIFECYCLE] cleanup incomplete stage=" + stage +
                " handler=" + handlerName + " detail=" + detail);
        }
