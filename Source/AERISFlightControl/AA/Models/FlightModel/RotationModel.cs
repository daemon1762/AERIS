...
namespace AtmosphereAutopilot
{
    ...
    public sealed partial class FlightModel : AutopilotModule
    {
        ...
        {
            var controlSurfaceType = AtmosphereAutopilot.control_surface_module_type;
            HasControlSurfaces = false;

            if (controlSurfaceType == null || vessel == null || vessel.Parts == null)
                return;

            // PartModuleList is not a generic IEnumerable on KSP 1.12 / Mono,
            // so use an explicit module scan rather than LINQ Any().
            foreach (Part part in vessel.Parts)
            {
                ...
                {
                    if (module != null && controlSurfaceType.IsAssignableFrom(module.GetType()))
                    {
