...
{
    // R041 HEIGHTMAP PQS INTEGRATION WITNESS.
    //
    // Main thread:
    // - discovers the real enabled stock PQSMod_VertexHeightMap for each target body;
    // - resolves the effective runtime MapSO semantics into a primitive snapshot;
    // - invokes the REAL OnVertexBuildHeight callback only against a newly-created,
    //   isolated PQS.VertexBuildData instance for every witness;
    // - copies only primitive expected bits / immutable snapshots to the worker.
    // Worker:
    // - never receives or dereferences Unity/KSP/runtime objects;
    // - evaluates the certified pure MapSO + HeightMap formula;
    // - requires exact IEEE-754 double-bit parity with the real callback.
    //
    // No live PQS/modifier field is changed. Production/DB authority remains PQS.
    [KSPAddon(KSPAddon.Startup.MainMenu, false)]
    internal sealed class AERIS39HeightMapPqsIntegrationWitnessObserver : MonoBehaviour
    {
        const string Candidate = "AERIS39_R041_HEIGHTMAP_PQS_INTEGRATION_WITNESS_V1";
        ...
        sealed class BodyCase
        {
            ...
            internal bool ScaleDeformityByRadius;
            internal double PublicDeformity;
            internal double EffectiveDeformity;
        ...
        BodyCase CaptureBody(string bodyName)
        {
            ...
            PQSMod_VertexHeightMap selected = null;
            int selectedIndex = -1;
            for (int i = 0; i < mods.Count; i++)
            {
                ...
                PQSMod_VertexHeightMap typed = raw as PQSMod_VertexHeightMap;
                if (typed == null)
                    throw new InvalidOperationException(bodyName + "_VERTEX_HEIGHT_MAP_CAST_FAILED");
                if (selected != null)
            ...
            double offset = selected.heightMapOffset;
            double publicDeformity = selected.heightMapDeformity;
            bool scaleByRadius = selected.scaleDeformityByRadius;
            double effectiveDeformity = ReadDoubleDefault(selected, "heightDeformity", publicDeformity);
            ...
            var snapshot = new AERIS39HeightMapPureCpuExact.Snapshot(
                offset, effectiveDeformity, mapSnapshot);

            List<CoordinateSample> coords = BuildSamples(width, height, bodyName, selectedIndex);
            double[] seedHeights = BuildSeedHeights(body.Radius, offset, effectiveDeformity);
            ...
            for (int c = 0; c < coords.Count; c++)
            {
                ...
                for (int h = 0; h < seedHeights.Length; h++)
                {
                    ...
                    CaptureCallbackReference(selected, check);
                    checks.Add(check);
                }
            }

            AERISLogger.Info(
                "[AERIS39][HEIGHTMAP_PQS_SNAPSHOT]" +
                "; candidate=" + Candidate +
                "; body=" + Safe(bodyName) +
                ...
                "; offset=" + R(offset) +
                "; public_deformity=" + R(publicDeformity) +
                "; effective_deformity=" + R(effectiveDeformity) +
                "; scale_deformity_by_radius=" + Bool(scaleByRadius) +
            ...
            return new BodyCase
            {
                ...
                ModifierOrder = order,
                ScaleDeformityByRadius = scaleByRadius,
                PublicDeformity = publicDeformity,
                EffectiveDeformity = effectiveDeformity,
        ...
        static void CaptureCallbackReference(
            PQSMod_VertexHeightMap mod,
            ExpectedCheck check)
        {
            try
            {
                ...
                data.u = check.U;
                data.v = check.V;
                data.vertHeight = check.InputHeight;
        ...
        void Report(WorkerResult result)
        {
            ...
            for (int i = 0; i < result.Bodies.Length; i++)
            {
                ...
                {
                    AERISLogger.Warn(
                        "[AERIS39][HEIGHTMAP_PQS_MISMATCH]" +
                        "; body=" + Safe(body.Name) +
            ...
            AERISLogger.Info(
                "[AERIS39][HEIGHTMAP_PQS_COMPLETE]" +
                "; pass=" + Bool(pass) +
                "; candidate=" + Candidate +
        ...
        static double ReadDoubleDefault(object target, string name, double fallback)
        {
            object raw = ReadMember(target, name);
            if (raw == null) return fallback;
