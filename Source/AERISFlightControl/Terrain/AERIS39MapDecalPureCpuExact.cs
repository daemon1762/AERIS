...
{
    // R041 pure CLR height evaluator for stock PQSMod_MapDecal.
    //
    // HARD RULE: no Unity/KSP/runtime-object types in this file. Runtime setup
    // state is snapshotted on the main thread as primitives plus immutable map bytes.
    internal static class AERIS39MapDecalPureCpuExact
    {
            ...
            AERISR041MohoDresPureCpuExact.HeightOpSnapshot
        {
                ...
                double height)
            {
                ...
                // Stock source converts the Vector3d direction to UnityEngine.Vector3
                // for Quaternion.operator*, then casts the binary32 result back to
                // Vector3d. Preserve that precision boundary exactly.
                float px = (float)x;
                float py = (float)y;
                float pz = (float)z;
                float rx;
                ...
                float smoothFactor = GetHeightSmoothing(
                    mapU, mapV, SmoothHeight, SmoothHR, SmoothH1M);
                ...
                // Stock cullBlack only suppresses height application when the map
                // height is not positive. The removeScatter side effect is intentionally
                // outside this height-only worker payload.
                if (CullBlack && !(ha.Height > 0f))
                    return height;

                double deformation = HeightMapDeformity * (double)ha.Height;
                double target;
                if (Absolute)
                    target = SphereRadius + AbsoluteOffset + deformation;
                else
                    target = height + deformation;

                // Stock PQSMod_MapDecal.Lerp(v2, v1, dt):
                // (v1 * dt) + (v2 * (1d - dt)).
                double dt = (double)smoothFactor;
                return (target * dt) + (height * (1d - dt));
            }
        }
        ...
        static void ConstructBilinearCoordsFloat(
            ...
            out float midY)
        {
            // Exact MapSO float-coordinate arithmetic. KSPCommunityFixes keeps X
            // periodic but clamps Y to [0, 0.99999f].
        ...
        static void RotateQuaternionVector(
            ...
            out float rz)
        {
            // UnityEngine.Quaternion.operator *(Quaternion, Vector3), preserving
            // managed binary32 operation order.
            float num = qx * 2f;
            float num2 = qy * 2f;
            float num3 = qz * 2f;
            float num4 = qx * num;
