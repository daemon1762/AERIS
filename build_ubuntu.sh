...
DISPLAY="AERIS Flight Control v$SEMVER DEV CP3.75 OPERATION HEALTH PASS 1 HOTFIX 1"
printf 'using System.Reflection;\n[assembly: AssemblyVersion("%s")]\n[assembly: AssemblyFileVersion("%s")]\nnamespace AERISFlightControl { internal static class AERISBuildVersion { internal const string Semantic = "%s"; internal const string Display = "%s"; internal const string UiCheckpoint = "DEV CP3.75 — OPERATION HEALTH PASS 1 HOTFIX 1"; internal const string Cp2FrozenBaselineDisplay = "%s"; } }\n' "$SEMVER" "$SEMVER" "$SEMVER" "$DISPLAY" "$BASELINE_DISPLAY" > "$GEN"
...
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT/Tools/run_v01800_operation_health_pass1_prebuild.py"
cd "$SRC"
