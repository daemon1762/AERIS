...
DISPLAY="AERIS Flight Control v$SEMVER DEV CP3.75 UNIFIED COASTAL BOUNDARY REL PALETTE AUTHORITY CANDIDATE 9"
printf 'using System.Reflection;\n[assembly: AssemblyVersion("%s")]\n[assembly: AssemblyFileVersion("%s")]\nnamespace AERISFlightControl { internal static class AERISBuildVersion { internal const string Semantic = "%s"; internal const string Display = "%s"; internal const string UiCheckpoint = "DEV CP3.75 — UNIFIED COASTAL BOUNDARY / REL PALETTE AUTHORITY CANDIDATE 9"; internal const string Cp2FrozenBaselineDisplay = "%s"; } }\n' "$SEMVER" "$SEMVER" "$SEMVER" "$DISPLAY" "$BASELINE_DISPLAY" > "$GEN"
...
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT/Tools/run_v01800_cp375_candidate9_prebuild.py"
cd "$SRC"
