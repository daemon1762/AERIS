...
if 'var vertices = new Vector3[vertexCount];\n            int[] indices = GetIdentityIndices(vertexCount);' not in s:
    ...
    s=s.replace(old,new,1)
# remove remaining line-mesh identity write only
...
# ProjectMesh: remove per-update bounds scan and make it instance for telemetry.
...
# DrawEntry: one terrain SetPass, same painter order.
...
# Water colour scratch, instance methods.
...
# Clear managed caches when GPU resources are explicitly released.
old='''            DestroyMeshPool();\n            entries.Clear();'''
