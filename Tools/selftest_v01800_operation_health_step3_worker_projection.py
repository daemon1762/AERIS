...
for forbidden in ('Mesh ', 'Material ', 'RenderTexture', 'Transform', 'Vessel', 'CelestialBody', 'Graphics.', 'GL.', 'UnityEngine.Object'):
    ck(forbidden not in worker,'worker math does not access native API: '+forbidden.strip())
ck('Vector3[] destination' in worker and 'new Vector3(u, v, 0f)' in worker,'worker output is value-array only')
...
ck('projectionWorkerCompleted=null;' in WF[WF.index('boolTryCommitProjectionWorkerResult'):WF.index('boolProjectionWorkerResultStillCurrent')] and WF.index('Time.realtimeSinceStartup-frontCommittedRealtime<ProjectionWorkerMinimumCommitIntervalSeconds') < WF.index('projectionWorkerCompleted=null;',WF.index('boolTryCommitProjectionWorkerResult')),'cadence-deferred completed result is retained rather than discarded')
...
ck('oh_project_worker_submit=' in W and 'oh_project_worker_commit=' in W and 'oh_project_worker_fallback=' in W and 'oh_project_worker_stale=' in W and 'oh_project_worker_defer=' in W and 'project_worker_ms=' in W,'runtime Worker Projection telemetry is published')
