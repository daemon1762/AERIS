...
# Preserve the accepted Stagger/Affine visual and cadence authorities.
...
require("RenderTextureFormat.ARGB32" in renderer and
        "FilterMode.Bilinear" in renderer,
...
# PENICILLIN passive monitor contract.
...
require("GC.CollectionCount(0)" in monitor and
        "GC.CollectionCount(1)" in monitor and
        "GC.CollectionCount(2)" in monitor,
        "generation GC events are observed")
require("Time.fixedDeltaTime" in monitor and
        "fixedSimSecondsWindow" in monitor and
        "physics_ratio_1s=" in monitor and
        "physics_ratio_5s=" in monitor,
        "physics realtime integrity is measured at 1s and 5s")
require("fixed_catchup_frames=" in monitor and
        "fixed_catchup_peak=" in monitor and
        "fixed_gap_max_ms=" in monitor,
        "FixedUpdate catch-up and wall-gap metrics are published")
require("frame_p95_ms=" in monitor and
        "frame_max_ms=" in monitor and
        "frame_hitch=" in monitor,
        "frame health p95/max/hitch metrics are published")
require("collision_nd_frame=" in monitor and
        "collision_nd_physics=" in monitor and
        "collision_nd_gc=" in monitor,
        "heavy-work collision telemetry is published")
require("aeris_known_main_max_ms=" in monitor and
        "attribution=PARTIAL_PASSIVE" in monitor,
        "AERIS main attribution is explicitly partial/passive")
require("oh_self_max_ms=" in monitor and
        "oh_self_ema_ms=" in monitor,
...
# Passive-only integration hooks.
...
require("penicillinBackMilliseconds" in renderer and
        "penicillinExactThisBack" in renderer,
...
# Config and install preservation.
require("codename = PENICILLIN" in config and
        "logLevel = DIAGNOSTIC" in config and
