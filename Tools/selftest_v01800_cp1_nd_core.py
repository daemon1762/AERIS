...
suite = CheckSuite("v0.18.0.0 CP1 ND core, PLAN and runway-display verification")
...
suite.check("RecordNavigationDisplayTextureUpload" in nd,
            "ND texture upload cost is measured")
...
for token in ("ND ranges are fixed at 5 / 10 / 20 / 40 / 80 / 160 km",
              "ND main EMA: layout"):
    suite.check(token in window, "SYSTEM options explain CP1 behavior: " + token)
