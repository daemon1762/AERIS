...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris48_r043_preload_ptc_production_cleanup2.sh <KSP root>" >&2
  exit 2
fi

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
KSP="$1"
EXPECTED_BRANCH="agent/aeris48-r043-preload-ptc-production-cleanup2"
ACCEPTED_SHA="8b9f48a8592dc6e4e2e241b69a65ffde21359991"
CANDIDATE="AERIS48_R043_PTC_PRODUCTION_CLEANUP2"

PROJECT_DIR="$ROOT/Source/AERISFlightControl"
PROJECT="$PROJECT_DIR/AERISFlightControl.csproj"
TEMP_PROJECT="$PROJECT_DIR/AERISFlightControl.R048PtcCleanup2.csproj"
TEMP_VERSION="$PROJECT_DIR/Properties/AERISBuildVersion.R048PtcCleanup2.generated.cs"
...
STATE_DIR="$HOME/.cache/AERIS/r048-ptc-cleanup2/$KEY"
...
static_gate(){
  ...
  local OLD="$PROJECT_DIR/Terrain/AERISR043PreloadPtcIntegratedShadow.cs"
  local PROD="$PROJECT_DIR/Terrain/AERISR047ExactCpuProduction.cs"
  local PIPE="$PROJECT_DIR/Terrain/AERISTerrainBlockPipeline.cs"

  [[ -f "$PTC" ]] || fail=$((fail+1))
  [[ ! -e "$OLD" ]] || fail=$((fail+1))
  grep -Fq 'AERISR043PreloadPtcPipeline.cs' "$PROJECT" || fail=$((fail+1))
  ...
  ! grep -Fq 'R043Shadow' "$PTC" || fail=$((fail+1))
  ! grep -Fq 'r043Shadow' "$PTC" || fail=$((fail+1))
  ! grep -Fq 'R043Shadow' "$PROD" || fail=$((fail+1))
  ! grep -Fq 'r043Shadow' "$PROD" || fail=$((fail+1))
  ! grep -Fq 'R043Shadow' "$PIPE" || fail=$((fail+1))
  ! grep -Fq 'r043Shadow' "$PIPE" || fail=$((fail+1))

  grep -Fq 'R043PtcTileState' "$PTC" || fail=$((fail+1))
  grep -Fq 'R043PtcBlockPayload' "$PTC" || fail=$((fail+1))
  grep -Fq 'R043Ptc' "$PIPE" || fail=$((fail+1))
  grep -Fq 'R047ShouldUseExactCpuProduction' "$PTC" || fail=$((fail+1))
  grep -Fq 'ProductionElevation = state.SamplingElevation' "$PTC" || fail=$((fail+1))
  grep -Fq 'R047TryCaptureExactCpuProductionSample' "$PIPE" || fail=$((fail+1))
  ...
  (( fail == 0 )) || {
    echo "STOP: cleanup2 static gate failed count=$fail" >&2
    exit 15
  }
}
harvest(){
  ...
  seg="$(mktemp /tmp/AERIS48_CLEANUP2.XXXXXX)"
  ...
  echo "=== AERIS48 CLEANUP2 RESTART VERIFICATION ==="
  ...
  if (( identity < 1 )); then
    echo "AERIS48_CLEANUP2_VERDICT=FAIL_NO_BUILD_IDENTITY"
    exit 41
  fi
  if (( fallback != 0 )); then
    echo "AERIS48_CLEANUP2_VERDICT=FAIL_EXACT_FALLBACK"
    exit 42
  fi
  if (( transitions != 0 )); then
    echo "AERIS48_CLEANUP2_VERDICT=FAIL_ENVIRONMENT_TRANSITION"
    exit 43
  fi
  if (( liveproof != 0 )); then
    echo "AERIS48_CLEANUP2_VERDICT=FAIL_LEGACY_LIVE_PROOF_ACTIVE"
    exit 44
  fi
  if [[ "$before_bytes" != "$after_bytes" ||
        "$before_files" != "$after_files" ||
        "$before_tree" != "$after_tree" ]]; then
    echo "AERIS48_CLEANUP2_VERDICT=FAIL_DB_MUTATION"
    exit 45
  fi

  echo "AERIS48_CLEANUP2_VERDICT=PASS"
  echo "AERIS_CURRENT_STAGE=PRELOAD_PTC_PRODUCTION_CLEANUP2_PASS"
  echo "next_action=Freeze PRELOAD_PTC accepted state and proceed to TERRAIN_ND."
}

cd "$ROOT"

echo "=== AERIS48 / R043 PRELOAD PTC PRODUCTION CLEANUP2 ==="
...
replacement=(
    '    <Compile Include="Properties\\AERISBuildVersion.R048PtcCleanup2.generated.cs" />\n'
...
echo "=== AERIS48 CLEANUP2 INSTALLED ==="
echo "HEAD=$HEAD_SHA"
...
echo "legacy_r043_shadow_identifiers=false"
