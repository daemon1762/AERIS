...
suites=[
 ...
 ('Source SHA-256 manifest verification','verify_cp35_gate3_candidate2_presentation_authority_hotfix1_source_manifest.py'),
 ('Package SHA-256 manifest verification','verify_cp35_gate3_candidate2_presentation_authority_hotfix1_package_manifest.py'),
]
for label,name in suites:
 print('\n=== '+label+' ===',flush=True)
 r=subprocess.run([sys.executable,str(root/'Tools'/name)],cwd=str(root))
 if r.returncode: raise SystemExit(r.returncode)
print('\n[v0.18.0.0 CP3.5 Gate 3 Candidate 2 Presentation Authority Hotfix 1] %d/%d acceptance suites PASS' % (len(suites),len(suites)))
