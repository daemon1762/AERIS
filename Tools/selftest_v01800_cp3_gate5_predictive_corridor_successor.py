...
suite = CheckSuite("v0.18.0.0 CP3 Gate 5 Predictive Corridor Frozen Successor")
...
suite.check("G/F/R/L/LD" not in ui and "rs.LandCount" not in ui and
            "rs.PinnedEntryCount" not in ui,
            "successor removes Resident debug line from SYSTEM")
suite.check("CP3 Corridor:" not in ui and "TurnRateDegPerSecond" not in ui and
            "LookAheadDistanceMeters" not in ui and "LandDemandActive" not in ui,
            "successor removes predictive corridor debug telemetry from SYSTEM")
suite.check(
            'UiCheckpoint = "DEV CP3 GATE 5 — INTEGRATED ACCEPTANCE CANDIDATE 7 — EXPANSION DETECTION / DLC RUNTIME STATUS HOTFIX 1"' in generated and
            'UiCheckpoint = "DEV CP3 GATE 5 — INTEGRATED ACCEPTANCE CANDIDATE 7 — EXPANSION DETECTION / DLC RUNTIME STATUS HOTFIX 1"' in build,
            "generated/build tab version names Gate 5 acceptance")
...
# Candidate 8 explicitly optimizes the metadata-only Map DRAM publish path. Preserve
# the Gate 3 ownership boundary semantically instead of freezing an obsolete byte hash.
suite.check("internal sealed class AERISMapTerrainIndexEntry" in map_dram and
            "internal sealed class AERISMapDramSnapshot" in map_dram and
            "AERISMapTerrainIndexEntry" in map_dram,
            "Map DRAM implementation remains metadata-only after approved performance successor")
...
# Candidate 12 adds one narrowly-authorized parser extension so shipped DLC
# directions can carry explicit UserCalibrated field authority.  Keep all other
# unrelated Landing code frozen against the established successor baseline.
def landing_hash_without_authorized_successor_deltas():
    base = ROOT / "Source/AERISFlightControl/Landing"
    excluded = {"AERISSandboxNativeSpawnWarpUtility.cs",
                "AERISAirfieldRegistry.cs",
                "AERISRunwayCertificationCache.cs",
                "AERISRunwayWitnessLibrary.cs",
                "AERISExpansionStatus.cs",
                "AERISAirfieldConfigParser.cs"}
    h = hashlib.sha256()
    for path in sorted(p for p in base.rglob("*") if p.is_file() and
                       p.name not in excluded):
        h.update(path.relative_to(ROOT).as_posix().encode("utf-8") + b"\0")
        h.update(hashlib.sha256(path.read_bytes()).digest())
    return h.hexdigest()

suite.check(landing_hash_without_authorized_successor_deltas() ==
            "f69b9e4c10923c7797eeda52970f8fb5c18afe871a1359079a03e9f743d72380",
            "unrelated Landing code remains byte-identical outside authorized successor deltas")
suite.check((SOURCE / "Landing/AERISSandboxNativeSpawnWarpUtility.cs").is_file(),
            "Candidate 5 retains the native spawn registration utility")

# Candidate 12 is authorized to change only the stock/DLC foundation and the
# field-verified calibration baseline.  The independent MOD survey catalog must
# remain byte-identical.
suite.check(hashlib.sha256((ROOT /
            "GameData/AERISFlightControl/Airfields/Defaults/02_Current_Mod_Runway_Survey_Catalog.cfg").read_bytes()).hexdigest() ==
            "51297fba743f7f913a689f017bc768c78cd71b4712d5b3d76ed02891d71cabe0",
            "unrelated MOD airfield survey catalog remains byte-identical")
