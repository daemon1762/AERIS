...
reuse_gate = draw.find('bool rev35R013ReuseStableSnapshot = !contentGeometryChanged &&')
drain = draw.find('DrainCompleted(system);')
...
check(reuse_gate >= 0, 'stable snapshot reuse gate exists')
check('contentSnapshotValid && visible != null && tiles != null &&' in draw[reuse_gate:reuse_gate + 320],
      'reuse requires a valid non-empty prior content snapshot')
check(drain >= 0 and drain < reuse_gate,
      'completion drain progresses before reuse/full-capture choice')
...
check('operationHealthRev35R013SnapshotReuses++;' in draw[reuse_branch:full_capture],
      'completion/retry reuse is directly observable')
check('operationHealthRev35R013CompletionReconciles++;' in draw[reuse_branch:full_capture],
      'worker-completion reconcile is directly observable')
check('operationHealthRev35R013RetryReconciles++;' in draw[reuse_branch:full_capture],
...
check(resolve > requested and schedule > resolve and measure > resolve,
      'shared resolve/schedule/foundation path remains after reuse gate')
...
anchor_slice = draw[anchor_guard:ready_assign] if anchor_guard >= 0 and ready_assign > anchor_guard else ''
for token in ('contentTerrainGeneration = visible.TerrainGeneration;',
    ...
    check(token in anchor_slice, 'true refresh owns planner anchor: ' + token)
...
check('contentSnapshotValid = true;' in draw and 'contentGpuReadyPending = true;' in draw,
...
check('verify_aeris28_rev3_5_salbutamol_r013_stable_content_snapshot_reconcile.py' in build,
      'R013 verifier wired into build')
check('selftest_v01800_oh_rev35_r013_stable_content_snapshot_reconcile.py' in prebuild,
      'R013 selftest wired into prebuild')
...
for forbidden in ('Task.Run(', 'new Thread(', 'ThreadPool.', 'WaitManagedPreparation',
    ...
    check(forbidden not in renderer, 'R013 renderer excludes ' + forbidden)

# Truth-table guard for the intended admission rule. Completion/retry work can reuse only
# a valid stable snapshot; any geometric change or missing snapshot must capture afresh.
...
check(reuse(False, True, True, True), 'truth table: stable completion/retry reuses snapshot')
check(not reuse(True, True, True, True), 'truth table: geometry change forces full capture')
check(not reuse(False, False, True, True), 'truth table: invalid snapshot forces full capture')
check(not reuse(False, True, False, True), 'truth table: missing visible set forces full capture')
check(not reuse(False, True, True, False), 'truth table: empty tile snapshot forces full capture')
