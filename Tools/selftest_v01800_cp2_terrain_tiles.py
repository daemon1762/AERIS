...
suite = CheckSuite("v0.18.0.0 CP2 terrain tile, cache and GPU-display foundation")
...
for token, source in (
    ...
    ("AERISTerrainTileLod.Land", system), ("30.0, 120.0, 420.0", system),
...
suite.check("OFF -> AUTO/ON" in renderer,
            "user-visible GPU mode toggle provides explicit retry")
suite.check("AUTO selected CPU fallback" in renderer and "GPU ON remains" in renderer,
            "AUTO and forced GPU ON have distinct capability semantics")
...
for token in ("DrawTerrainMap", "terrainTileRenderer.Draw", "CPU FALLBACK", "TERR OFF",
