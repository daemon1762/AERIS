...
suite = CheckSuite("v0.18.0.0 CP3 Gate 3 Predictive Forward Corridor")
...
runner = read(ROOT / "Tools/run_v01800_cp3_gate3_acceptance.py")
...
suite.check("acceptedCorridorCount" in plan and
            "Math.Min(maximum, requestScratch.Count)" in plan,
            "corridor requests remain inside the existing terrain request cap")
...
suite.check("return IsBackgroundPopulationLod(lod) ||" in tiles and
            "lod == AERISTerrainTileLod.Land" in tiles[tiles.find(
...
suite.check((
            'UiCheckpoint = "DEV CP3 GATE 3 — PREDICTIVE FORWARD CORRIDOR"' in generated and
            'UiCheckpoint = "DEV CP3 GATE 3 — PREDICTIVE FORWARD CORRIDOR"' in build) or (
            'UiCheckpoint = "DEV CP3 GATE 3 — TERRAIN CONTINUITY & TELEMETRY HOTFIX 1"' in generated and
            'UiCheckpoint = "DEV CP3 GATE 3 — TERRAIN CONTINUITY & TELEMETRY HOTFIX 1"' in build),
            "generated/build tab version names Gate 3 or its accepted continuity successor")
...
suite.check("CP3 Gate 3 predicts a bounded constant-turn-rate forward corridor" in bootstrap,
            "startup log states the live Gate 3 route and boundary")
...
suite.check("selftest_v01800_cp3_gate3_predictive_forward_corridor.py" in runner,
            "active Gate 3 runner executes the dedicated test")
suite.check("run_v01800_cp3_gate3_acceptance.py" in build or
            "run_v01800_cp3_gate3_terrain_continuity_telemetry_hotfix1_acceptance.py" in build,
            "build entrypoint runs Gate 3 acceptance or its continuity successor")
