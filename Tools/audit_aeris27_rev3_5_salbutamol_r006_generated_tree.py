...
if not R.is_file() or not OH.is_file():
    raise SystemExit(PREFIX + ' renderer/OH missing')
text = R.read_text()
oh = OH.read_text()
for marker in (
    ...
    if marker not in text:
        raise SystemExit(PREFIX + ' missing renderer marker: ' + marker)
if 'AERIS26_REV003_OBSERVER_M1' not in oh:
    raise SystemExit(PREFIX + ' missing OH observer marker')
...
for signature in signatures:
    ...
    for line in body.splitlines():
        if any(token in line for token in (
            ...
            'allowPublication', 'Finalize', 'new ', '.Clone(', 'Capacity',
            'frontBufferSwaps', 'frontCommittedRealtime', 'renderReadyFields',
            'pendingEntryCommit')):
            print(line)

print('\n===== ALLOCATION SITE COUNTS =====')
for pattern in (
    r'new GeographicUnitPoint\[', r'new Vector3\[', r'new Color32\[', r'new int\[',
    r'\.Clone\(\)', r'\.ToArray\(\)', r'\.Capacity ='):
...
for token in (
    ...
    'deferredEntryRetirements', 'presentationEntryPins',
