...
suite = CheckSuite("v0.17.0.5 provider identity / geometry signature split static verification")
version_txt, version_json, version_cs = parse_version()
suite.equal(version_txt, "0.17.0.5", "VERSION is v0.17.0.5")
...
suite.check("CurrentAlgorithmVersion = 1650" in contracts_code,
            "canonical cache fingerprint has a new algorithm version")
for token in ("StableProviderFingerprintIdentity(record)",
              "BuildPrimitiveAggregate(primitives", "BuildPointAggregate(points",
              "AccumulateFingerprint(hash", "QuantizedFingerprintValue(value, quantum)"):
    suite.check(token in snapshot_builder_code,
                "canonical cache fingerprint contract: " + token)
for forbidden in ("Append(builder, record.ProviderUuid);",
                  "Append(builder, value.SourceGroup);",
                  "int pointStep = Math.Max(1, points.Count / 128)"):
    suite.check(forbidden not in snapshot_builder_code,
