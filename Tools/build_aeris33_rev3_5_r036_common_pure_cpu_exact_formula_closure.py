...
PREFIX='[AERIS33 R036 COMMON PURE CPU EXACT FORMULA CLOSURE BUILD]'
...
parent=ROOT/'Source/AERISFlightControl/Terrain/AERISR035LandControlWriteSemanticsIlAuditObserver.cs'
...
if not parent.is_file() or PARENT not in version.read_text() or 'AERISR035LandControlWriteSemanticsIlAuditObserver.cs' not in csproj.read_text():
    raise SystemExit(PREFIX+' accepted R035 materialization missing. Do NOT reset/clean/stash. Materialize/build accepted R035 first, then return to R036.')
print(PREFIX+' PASS accepted R035 materialization reused')
frozen=[
...
run([sys.executable,ROOT/'Tools/apply_aeris33_rev3_5_r036_common_pure_cpu_exact_formula_closure.py'])
run([sys.executable,ROOT/'Tools/verify_aeris33_rev3_5_r036_common_pure_cpu_exact_formula_closure.py'])
for p in frozen:
...
for token in (MARKER,'[R036][COMMON_CPU] event=FORMULA_CLOSURE_WORKER_COMPLETE',
...
+'source_head='+head+'\n'
+'parent='+PARENT+'\n'
+'runtime_change=COMMON_PURE_CPU_EXACT_FORMULA_CLOSURE_SHADOW_ONLY\n'
+'targets=Minmus,Ike,Gilly,Pol\n'
+'expected_worker_ready=Gilly,Ike\n'
+'expected_formula_pending=Minmus,Pol\n'
...
print('TEST: restart KSP to Main Menu and wait for [R036][COMMON_CPU] event=FORMULA_CLOSURE_WORKER_COMPLETE')
