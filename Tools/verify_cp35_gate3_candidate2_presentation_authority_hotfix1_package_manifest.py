...
suite=CheckSuite('CP3.5 Gate 3 Candidate 2 Presentation Authority Hotfix 1 package manifest verification')
...
if manifest.is_file():
    for number,line in enumerate(manifest.read_text(encoding='utf-8').splitlines(),1):
        ...
        if len(parts)!=2 or len(parts[0])!=64:
            suite.check(False,'package manifest line '+str(number)+' format',line); continue
        expected[parts[1].removeprefix('./')]=parts[0].lower()
actual=sorted(str(p.relative_to(ROOT)).replace('\\','/') for p in ROOT.rglob('*')
    if p.is_file() and p.name!='MANIFEST_SHA256.txt' and '__pycache__' not in p.parts
...
suite.check(not bad,'all package manifest hashes match',', '.join(bad[:10]))
suite.finish()
