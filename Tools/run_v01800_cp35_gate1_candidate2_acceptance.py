...
tests=[
 ...
 'selftest_v01800_cp35_gate1_back_render_profiler_symmetric_top_ui_candidate2.py',
]
for name in tests:
    print('\n=== '+name+' ===',flush=True)
    r=subprocess.run([sys.executable,str(root/'Tools'/name)],cwd=str(root))
    if r.returncode: raise SystemExit(r.returncode)
print('\n[v0.18.0.0 CP3.5 Gate 1 Back Render Profiler / Symmetric Top UI Candidate 2] %d/%d scripts PASS' % (len(tests),len(tests)))
