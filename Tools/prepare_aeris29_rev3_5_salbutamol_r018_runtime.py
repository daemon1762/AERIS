...
PREFIX = '[AERIS29 REV3.5 SALBUTAMOL SULFATE R018 RUNTIME]'
...
parser = argparse.ArgumentParser(
    description='Reconstruct formal R017 lineage, overlay R018 visible-foundation presentation-gate split, verify, build and install once.')
...
if not O15.is_file() or R015 not in O15.read_text():
    raise SystemExit(RED + PREFIX + ' committed R015 observer source missing' + RESET)
if not O17.is_file() or R017 not in O17.read_text():
    raise SystemExit(RED + PREFIX + ' committed R017 observer source missing' + RESET)
if not REC.is_file():
    raise SystemExit(RED + PREFIX + ' recorder source missing' + RESET)
...
if not has(R, R005):
    ...
    if base_build.count(LEGACY_INSTALL_ANCHOR) != 1:
        raise SystemExit(RED + PREFIX +
            ' PRECHECK FAIL: legacy AERIS23 installed-DLL identity anchor is not byte-exact. Restore generated residue / update branch before reconstruction.' + RESET)
...
if has(R, R013) or 'REV3_5_R013_VARIANT=' in B.read_text():
    raise SystemExit(RED + PREFIX + ' R013 REJECTED EXPERIMENT LEAKED INTO R018 PARENT' + RESET)

stages = (
    ('apply_aeris28_rev3_5_salbutamol_r014_publication_gated_content_reconcile.py',
     'verify_aeris28_rev3_5_salbutamol_r014_publication_gated_content_reconcile.py'),
    ('apply_aeris29_rev3_5_salbutamol_r015_periodic_gc_attribution_observer.py',
     'verify_aeris29_rev3_5_salbutamol_r015_periodic_gc_attribution_observer.py'),
    ('apply_aeris29_rev3_5_salbutamol_r016_fdr_high_rate_diagnostics_isolation.py',
     'verify_aeris29_rev3_5_salbutamol_r016_fdr_high_rate_diagnostics_isolation.py'),
    ('apply_aeris29_rev3_5_salbutamol_r017_nd_presentation_stall_observer.py',
     'verify_aeris29_rev3_5_salbutamol_r017_nd_presentation_stall_observer.py'),
    ('apply_aeris29_rev3_5_salbutamol_r018_visible_foundation_presentation_gate_split.py',
     'verify_aeris29_rev3_5_salbutamol_r018_visible_foundation_presentation_gate_split.py'),
)
for apply_script, verify_script in stages:
    run([sys.executable, ROOT / 'Tools' / apply_script])
    run([sys.executable, ROOT / 'Tools' / verify_script])

run(['git', 'diff', '--check'])

for directory in (ROOT / 'Source/AERISFlightControl/bin',
...
for path in (source, installed, identity):
    if not path.is_file():
        raise SystemExit(RED + PREFIX + ' installed artifact missing: ' +
            str(path) + RESET)
...
checks = [
    ...
    (('rev3_5_r015_variant=' + R015) in ident, 'R015 identity retained'),
    (('rev3_5_r016_variant=' + R016) in ident, 'R016 identity retained'),
    (('rev3_5_r017_variant=' + R017) in ident, 'R017 identity retained'),
    (('rev3_5_r018_variant=' + R018) in ident, 'R018 identity installed'),
    ...
    (marker_in_bytes(dll, R014), 'DLL embeds R014 marker'),
    (marker_in_bytes(dll, R015), 'DLL embeds R015 marker'),
    (marker_in_bytes(dll, R016), 'DLL embeds R016 marker'),
    (marker_in_bytes(dll, R017), 'DLL embeds R017 marker'),
    (marker_in_bytes(dll, R018), 'DLL embeds R018 marker'),
    (marker_in_bytes(dll, 'MeasureVisibleFoundationGpuReadiness'),
     'DLL embeds R018 visible readiness helper'),
    (marker_in_bytes(dll, 'operationHealthRev35R018VisibleRequiredFar'),
     'DLL embeds R018 visible-required counter'),
    (marker_in_bytes(dll, 'operationHealthRev35R018OverscanHolAvoided'),
     'DLL embeds R018 HOL avoidance counter'),
    (marker_in_bytes(dll, 'oh_rev35_r018_visible_required_far='),
     'DLL embeds R018 visible telemetry field'),
    (marker_in_bytes(dll, 'oh_rev35_r018_overscan_hol_avoided='),
     'DLL embeds R018 HOL telemetry field'),
...
for ok, label in checks:
    print((GREEN if ok else RED) + ('[PASS] ' if ok else '[FAIL] ') +
          label + RESET)
    if not ok:
        failed.append(label)
if failed:
    raise SystemExit(RED + PREFIX + ' INSTALL IDENTITY FAIL: ' +
        ', '.join(failed) + RESET)
...
print(YELLOW + 'R018 CONTRACT:' + RESET +
      ' hidden 1.35x/250km overscan remains preparation-only. The existing Gate3.1 viewport-foundation planner is re-evaluated at the exact user-visible range only during full content reconcile; its guarded FAR keys require exact-current GPU Entries before FRONT swap or no-blank recovery. Hidden overscan CPU/GPU misses no longer head-of-line block presentation. Fixed 10Hz, exact 160km, visible complete-coverage truth, R014 publication batching, R016 isolation and R017 observer remain unchanged.')
