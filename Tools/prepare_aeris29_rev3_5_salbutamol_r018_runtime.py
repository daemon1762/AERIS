...
if not has(R, R005):
    ...
    if base_build.count(LEGACY_INSTALL_ANCHOR) != 1:
        raise SystemExit(RED + PREFIX +
            ' PRECHECK FAIL: legacy AERIS23 installed-DLL identity anchor is not byte-exact. Restore generated residue / update branch before reconstruction.' +
            RESET)
...
checks = [
    ...
    (marker_in_bytes(dll, 'OH_REV3_5_R011_TURN_CHURN'),
     'DLL embeds R011 observer marker'),
    ...
    (marker_in_bytes(dll, 'OH_REV3_5_R017_ND_PRESENT_STALL'),
     'DLL embeds R017 stall telemetry prefix'),
...
for ok, label in checks:
    print((GREEN if ok else RED) +
          ('[PASS] ' if ok else '[FAIL] ') + label + RESET)
    if not ok:
        failed.append(label)
if failed:
    raise SystemExit(RED + PREFIX +
        ' INSTALL IDENTITY FAIL: ' + ', '.join(failed) + RESET)
