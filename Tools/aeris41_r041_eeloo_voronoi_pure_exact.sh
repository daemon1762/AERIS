...
[[ -f "$STATE_IDENTITY_INJECTOR" ]] || { echo "STOP: runtime state-identity injector missing" >&2; exit 18; }
[[ -f "$VORONOI_PURE" ]] || { echo "STOP: VertexVoronoi pure source missing" >&2; exit 19; }
[[ -f "$HEIGHTNOISE_PURE" ]] || { echo "STOP: VertexHeightNoiseVertHeight pure source missing" >&2; exit 20; }
