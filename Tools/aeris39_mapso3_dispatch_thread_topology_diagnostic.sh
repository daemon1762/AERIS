...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris39_mapso3_dispatch_thread_topology_diagnostic.sh <KSP root>" >&2
  exit 2
fi
...
CANDIDATE="AERIS39_MAPSO3_DISPATCH_THREAD_TOPOLOGY_DIAGNOSTIC_V1"
...
TMP_CSPROJ="$SRC/.AERIS39_MAPSO3D.csproj"
...
OUT="$ARTIFACT_ROOT/AERIS39_MAPSO3D_Dispatch_Thread_Topology"
ARCHIVE="$ARTIFACT_ROOT/AERIS39_MAPSO3D_Dispatch_Thread_Topology.tar.gz"

KEY="$(printf '%s' "$KSP" | sha256sum | awk '{print substr($1,1,16)}')"
STATE_DIR="$HOME/.cache/AERIS/mapso3-dispatch-thread-topology/$KEY"
...
test -z "$(git status --porcelain)" || {
  echo "STOP: worktree dirty before MAPSO-3D" >&2
  git status -sb
  exit 6
}
...
write_artifacts() {
  ...
  grep '\[AERIS39\]\[MAPSO3D_' "$segment" > "$OUT/mapso3d_runtime_excerpt.txt" || true
...
pure_static_source_sha256=$(sha256sum "$SRC/Terrain/AERIS39MapSoPureCpuExact.cs" | awk '{print $1}')
topology_probe_source_sha256=$(sha256sum "$SRC/Terrain/AERIS39MapSoPureCpuTopologyProbe.cs" | awk '{print $1}')
diagnostic_observer_source_sha256=$(sha256sum "$SRC/Terrain/AERIS39MapSoDispatchThreadTopologyDiagnosticObserver.cs" | awk '{print $1}')
...
EOF
  ...
  (
    cd "$OUT"
    sha256sum mapso3d_runtime_excerpt.txt provenance.txt > SHA256SUMS.txt
  )

  rm -f "$ARCHIVE"
  tar -C "$ARTIFACT_ROOT" -czf "$ARCHIVE" AERIS39_MAPSO3D_Dispatch_Thread_Topology
}
...
harvest_if_ready() {
  ...
  segment="$(mktemp /tmp/AERIS39_MAPSO3D_segment.XXXXXX)"
  ...
  if grep -Fq "[AERIS39][MAPSO3D_FAIL]" "$segment"; then
    write_artifacts "$segment" "FAIL" "$installed_sha"
    echo "=== MAPSO-3D RUNTIME FAILURE ==="
    grep '\[AERIS39\]\[MAPSO3D_' "$segment" || true
  ...
  if ! grep -Fq "[AERIS39][MAPSO3D_COMPLETE]" "$segment"; then
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
    echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
    return 0
  ...
  local complete
  complete="$(grep -F "[AERIS39][MAPSO3D_COMPLETE]" "$segment" | tail -n 1)"
  ...
  for body in Kerbin Eve Duna Dres Moho Eeloo; do
    grep -F "[AERIS39][MAPSO3D_BODY]" "$segment" | grep -Fq "; body=$body;" || pass=0
  done

  write_artifacts "$segment" "$([[ "$pass" -eq 1 ]] && echo PASS || echo FAIL_ACCEPTANCE)" "$installed_sha"

  if [[ "$pass" -ne 1 ]]; then
    echo "=== MAPSO-3D ACCEPTANCE FAILURE ==="
    grep '\[AERIS39\]\[MAPSO3D_' "$segment" || true
    echo "archive=$ARCHIVE"
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=FAIL"
  ...
  echo "=== MAPSO-3D DIAGNOSTIC ==="
  grep '\[AERIS39\]\[MAPSO3D_MAP\]' "$segment" || true
  grep '\[AERIS39\]\[MAPSO3D_BODY\]' "$segment" || true
  grep '\[AERIS39\]\[MAPSO3D_DETAIL\]' "$segment" | head -n 24 || true
  echo "$complete"
  echo
  ...
  echo "AERIS39_MAPSO3D_CAPTURE=PASS"
  echo "AERIS_CURRENT_STAGE=PASS"
  echo "next=MAPSO-3_DIAGNOSTIC_ANALYSIS"
  return 0
}
...
if [[ -f "$STATE" ]]; then
  ...
  echo "INFO: stale MAPSO-3D state replaced for current HEAD"
  rm -rf "$STATE_DIR"
fi
...
insert = (
    ...
    "    <Compile Include=\"Terrain\\AERIS39MapSoPureCpuTopologyProbe.cs\" />\n"
    "    <Compile Include=\"Terrain\\AERIS39MapSoDispatchThreadTopologyDiagnosticObserver.cs\" />\n"
    "</ItemGroup><Import Project=\"$(MSBuildToolsPath)\\Microsoft.CSharp.targets\" /></Project>"
)
if needle not in src:
    raise SystemExit("MAPSO3D project insertion point not found")
...
(
  cd "$SRC"
  xbuild /p:Configuration=Release /p:KSPDIR="$KSP" .AERIS39_MAPSO3D.csproj
)
...
echo "AERIS39_MAPSO3D_BUILD_INSTALL=PASS"
echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
