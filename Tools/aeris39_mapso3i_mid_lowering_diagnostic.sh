...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris39_mapso3i_mid_lowering_diagnostic.sh <KSP root>" >&2
  exit 2
fi
...
CANDIDATE="AERIS39_MAPSO3I_MID_LOWERING_DIAGNOSTIC_V1"
...
TMP_CSPROJ="$SRC/.AERIS39_MAPSO3I.csproj"
...
OUT="$ARTIFACT_ROOT/AERIS39_MAPSO3I_Mid_Lowering_Diagnostic"
ARCHIVE="$ARTIFACT_ROOT/AERIS39_MAPSO3I_Mid_Lowering_Diagnostic.tar.gz"

KEY="$(printf '%s' "$KSP" | sha256sum | awk '{print substr($1,1,16)}')"
STATE_DIR="$HOME/.cache/AERIS/mapso3i-mid-lowering/$KEY"
...
test -z "$(git status --porcelain)" || {
  echo "STOP: worktree dirty before MAPSO-3I" >&2
  git status -sb
  exit 6
}
...
write_artifacts() {
  ...
  rm -rf "$OUT"
  mkdir -p "$OUT"
  grep '\[AERIS39\]\[MAPSO3I_' "$segment" > "$OUT/mapso3i_runtime_excerpt.txt" || true
...
diagnostic_observer_source_sha256=$(sha256sum "$SRC/Terrain/AERIS39MapSoMidLoweringDiagnosticObserver.cs" | awk '{print $1}')
...
EOF
  (cd "$OUT" && sha256sum mapso3i_runtime_excerpt.txt provenance.txt > SHA256SUMS.txt)
  rm -f "$ARCHIVE"
  tar -C "$ARTIFACT_ROOT" -czf "$ARCHIVE" AERIS39_MAPSO3I_Mid_Lowering_Diagnostic
}
...
harvest_if_ready() {
  ...
  segment="$(mktemp /tmp/AERIS39_MAPSO3I_segment.XXXXXX)"
  ...
  if grep -Fq "[AERIS39][MAPSO3I_FAIL]" "$segment"; then
    write_artifacts "$segment" "FAIL" "$installed_sha"
    echo "=== MAPSO-3I RUNTIME FAILURE ==="
    grep '\[AERIS39\]\[MAPSO3I_' "$segment" || true
  ...
  if ! grep -Fq "[AERIS39][MAPSO3I_COMPLETE]" "$segment"; then
    local boot_seen=false
    grep -Fq "[AERIS39][MAPSO3I_BOOT]" "$segment" && boot_seen=true
    rm -f "$segment"
  ...
  complete="$(grep -F "[AERIS39][MAPSO3I_COMPLETE]" "$segment" | tail -n 1)"
  ...
  for body in Kerbin Eve Duna Dres Moho Eeloo; do
    grep -F "[AERIS39][MAPSO3I_BODY]" "$segment" | grep -Fq "; body=$body;" || pass=0
  done

  write_artifacts "$segment" "$([[ "$pass" -eq 1 ]] && echo PASS || echo FAIL_ACCEPTANCE)" "$installed_sha"
  if [[ "$pass" -ne 1 ]]; then
    echo "=== MAPSO-3I ACCEPTANCE FAILURE ==="
    grep '\[AERIS39\]\[MAPSO3I_' "$segment" || true
    echo "archive=$ARCHIVE"
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=FAIL"
  ...
  echo "=== MAPSO-3I MID LOWERING DIAGNOSTIC ==="
  grep '\[AERIS39\]\[MAPSO3I_BODY\]' "$segment" || true
  grep '\[AERIS39\]\[MAPSO3I_DETAIL\]' "$segment" | head -n 48 || true
  echo "$complete"
  echo
  ...
  rm -f "$segment"
  rm -rf "$STATE_DIR"
  echo "AERIS39_MAPSO3I_CAPTURE=PASS"
  echo "AERIS_CURRENT_STAGE=PASS"
  echo "next=MAPSO-3I_MID_LOWERING_ANALYSIS"
  return 0
}

if [[ -f "$STATE" ]]; then
  if harvest_if_ready; then exit 0; fi
  echo "INFO: stale MAPSO-3I state replaced for current HEAD"
  rm -rf "$STATE_DIR"
fi
...
insert = (
    "    <Compile Include=\"Terrain\\AERIS39MapSoMidLoweringDiagnosticObserver.cs\" />\n"
    "</ItemGroup><Import Project=\"$(MSBuildToolsPath)\\Microsoft.CSharp.targets\" /></Project>"
)
if needle not in src:
    raise SystemExit("MAPSO3I project insertion point not found")
pathlib.Path(sys.argv[2]).write_text(src.replace(needle, insert, 1), encoding="utf-8")
PY

rm -rf "$SRC/bin/Release" "$SRC/obj/Release"
(cd "$SRC" && xbuild /p:Configuration=Release /p:KSPDIR="$KSP" .AERIS39_MAPSO3I.csproj)
...
echo "AERIS39_MAPSO3I_BUILD_INSTALL=PASS"
echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
