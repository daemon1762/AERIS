...
from v01630_testlib import ROOT, CheckSuite, sha256

suite = CheckSuite("v0.16.3.0 internal manifest verification")
...
suite.equal(sorted(expected), actual_paths, "manifest file set matches package")
bad = [relative for relative in actual_paths if expected.get(relative) != sha256(ROOT / relative)]
