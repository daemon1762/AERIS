...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris43_r043_preload_ptc_shadow_integration.sh <KSP root>" >&2
  exit 2
fi

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
KSP="$1"
EXPECTED_BRANCH="agent/aeris43-r043-preload-ptc-shadow-integration"
TILEGRID_ACCEPTED="12592479d0852e288444ba53405a3332ed247c60"
...
CANDIDATE_NAME="AERIS43_R043_PRELOAD_PTC_SHADOW_INTEGRATION"

PROJECT_DIR="$ROOT/Source/AERISFlightControl"
PROJECT="$PROJECT_DIR/AERISFlightControl.csproj"
TEMP_PROJECT="$PROJECT_DIR/AERISFlightControl.R043PreloadPtcShadowIntegration.csproj"
TEMP_VERSION="$PROJECT_DIR/Properties/AERISBuildVersion.R043PreloadPtcShadowIntegration.generated.cs"
...
HARNESS="$PROJECT_DIR/Terrain/AERISR043PreloadPtcIntegratedPipelineObserver.cs"
...
STATE_DIR="$HOME/.cache/AERIS/r043-preload-ptc-shadow-integration/$KEY"
...
echo "=== AERIS43 / R043 PRELOAD PTC SHADOW INTEGRATION ==="
...
for file in "$PROJECT" "$PIPELINE" "$INTEGRATION" "$HARNESS"   "$IDENTITY_OBSERVER" "$PRELOAD" "$DATABASE" "$TILE_SYSTEM" "$AWARENESS"   "$RESOLVER" "$SNAPSHOT" "$SNAPSHOT_BUILDER"; do
...
# Phase2 changes only the real BlockPipeline shadow seam plus its compiled partial.
# The PreloadBuilder scheduler, DB, TileSystem, TerrainAwareness and R042 exact
# certification sources must remain byte-identical to the accepted tile-grid proof.
for protected_path in   Source/AERISFlightControl/Terrain/AERISTerrainPreloadBuilder.cs   Source/AERISFlightControl/Terrain/AERISTerrainPreloadDatabase.cs   Source/AERISFlightControl/Terrain/AERISTerrainTileSystem.cs   Source/AERISFlightControl/Terrain/AERISTerrainAwareness.cs   Source/AERISFlightControl/Terrain/AERISR042ExactCpuShadowSourceResolver.cs   Source/AERISFlightControl/Terrain/AERISR042ExactCpuShadowRuntimeSnapshot.cs   Source/AERISFlightControl/Terrain/AERISR042ExactCpuShadowRuntimeSnapshotBuilder.cs; do
  if ! git diff --quiet "$TILEGRID_ACCEPTED" HEAD -- "$protected_path"; then
    echo "STOP: protected path changed in Phase2: $protected_path" >&2
    exit 18
  fi
done

# No unexpected source path may be changed relative to the accepted Phase1 point.
while IFS= read -r path; do
  case "$path" in
    Source/AERISFlightControl/AERISFlightControl.csproj|    Source/AERISFlightControl/Terrain/AERISTerrainBlockPipeline.cs|    Source/AERISFlightControl/Terrain/AERISR043PreloadPtcIntegratedShadow.cs|    Source/AERISFlightControl/Terrain/AERISR043PreloadPtcIntegratedPipelineObserver.cs|    Tools/aeris.sh|    Tools/aeris_current_stage.sh|    Tools/aeris43_r043_preload_ptc_shadow_integration.sh)
      ;;
    "")
      ;;
    *)
      echo "STOP: unexpected Phase2 changed path: $path" >&2
      exit 19
      ;;
  esac
done < <(git diff --name-only "$TILEGRID_ACCEPTED" HEAD)

grep -Fq 'internal sealed partial class AERISTerrainBlockPipeline' "$PIPELINE" || exit 20
grep -Fq 'TryCreateR043ShadowState(body, request, pqsHash)' "$PIPELINE" || exit 21
grep -Fq 'CaptureR043ShadowSample(' "$PIPELINE" || exit 22
grep -Fq 'ProcessR043ShadowBlock(' "$PIPELINE" || exit 23
grep -Fq 'ReportR043ShadowTile(state)' "$PIPELINE" || exit 24
grep -Fq 'request.WorkOwner != AERISTerrainWorkOwner.PreloadBuilder' "$INTEGRATION" || exit 25
grep -Fq 'AERISR042ExactCpuShadowRuntimeSnapshotBuilder.TryCapture' "$INTEGRATION" || exit 26
grep -Fq 'AERISR039MinmusPureCpuExact.EvaluateVertexPlanet' "$INTEGRATION" || exit 27
grep -Fq 'AERIS39AllBodyHeightModifierChainPureCpuExact.Evaluate' "$INTEGRATION" || exit 28
grep -Fq 'tile_commit_authority=PQS' "$INTEGRATION" || exit 29
grep -Fq 'exact_cpu_db_write=false' "$INTEGRATION" || exit 30
grep -Fq 'worker_runtime_object_access=false' "$INTEGRATION" || exit 31
grep -Fq 'internal const bool ProducerSwitchEnabled = false;' "$RESOLVER" || exit 32
grep -Fq 'Terrain\AERISR043PreloadPtcIntegratedShadow.cs' "$PROJECT" || exit 33

if grep -Eq 'new[[:space:]]+Thread|Task\.|ThreadPool\.|Parallel\.' "$INTEGRATION"; then
  echo "STOP: integrated shadow contains private worker path" >&2
  exit 34
fi
if grep -Eq '\.OnVertexBuildHeight[[:space:]]*\(' "$INTEGRATION"; then
  echo "STOP: integrated shadow invokes live height callback" >&2
  exit 35
fi
if grep -Eq 'AERISTerrainPreload(Database|Builder).*\.(Write|Commit|Save|Store|Put)' "$INTEGRATION"; then
  echo "STOP: integrated exact CPU path contains DB mutation" >&2
  exit 36
fi
...
observer_needle = '    <Compile Include="Terrain\\AERISR043PreloadPtcIntegratedShadow.cs" />\n'
observer_insert = (
    observer_needle +
    '    <Compile Include="Terrain\\AERISR043PreloadPtcIntegratedPipelineObserver.cs" />\n' +
...
version_replacement = '    <Compile Include="Properties\\AERISBuildVersion.R043PreloadPtcShadowIntegration.generated.cs" />\n'
if text.count(observer_needle) != 1:
    raise SystemExit('R043 integration insertion marker not unique')
if text.count(version_needle) != 1:
    raise SystemExit('R043 version replacement marker not unique')
if 'AERISR043PreloadPtcIntegratedPipelineObserver.cs' in text:
    raise SystemExit('integration harness unexpectedly in canonical csproj')
...
grep -Fq 'AERISR043PreloadPtcIntegratedPipelineObserver.cs' "$TEMP_PROJECT" || exit 40
grep -Fq 'AERISR043RuntimeBuildIdentityObserver.cs' "$TEMP_PROJECT" || exit 41
grep -Fq 'AERISBuildVersion.R043PreloadPtcShadowIntegration.generated.cs' "$TEMP_PROJECT" || exit 42
...
harvest_if_ready() {
  ...
  [[ -f "$LOG" ]] || {
    ...
    echo "human_action=Launch KSP to Main Menu, wait for integration completion, exit KSP, then run the same command again."
    return 0
  }
  ...
  segment="$(mktemp /tmp/AERIS43_R043_PTC_INTEGRATION.XXXXXX)"
  ...
  complete="$(grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATION_HARNESS_COMPLETE]' "$segment" | tail -n 1 || true)"
  ...
  [[ "$complete" == *'; tiles=7;'* ]] || pass=0
  [[ "$complete" == *'; resolution=17;'* ]] || pass=0
  [[ "$complete" == *'; expected_total_checks=2023;'* ]] || pass=0
  [[ "$complete" == *'; commit_target=RAM_CALLBACK_ONLY;'* ]] || pass=0
  ...
  local tile_lines commit_lines
  tile_lines="$(grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATED_TILE]' "$segment" | grep -F '; proof_request=true;' | wc -l | tr -d ' ')"
  commit_lines="$(grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATION_HARNESS_COMMIT]' "$segment" | wc -l | tr -d ' ')"
  [[ "$tile_lines" = "7" ]] || pass=0
  [[ "$commit_lines" = "7" ]] || pass=0

  for body in Minmus Kerbin Eve Duna Dres Moho Eeloo; do
    local line commit
    line="$(grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATED_TILE]' "$segment" |
      grep -F '; proof_request=true;' | grep -F "; body=$body;" | tail -n 1 || true)"
    commit="$(grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATION_HARNESS_COMMIT]' "$segment" |
      grep -F "; body=$body;" | tail -n 1 || true)"
    [[ -n "$line" && -n "$commit" ]] || { pass=0; continue; }
    [[ "$line" == *'; pass=true;'* ]] || pass=0
    [[ "$line" == *'; resolution=17;'* ]] || pass=0
    [[ "$line" == *'; checks=289;'* ]] || pass=0
    [[ "$line" == *'; expected_checks=289;'* ]] || pass=0
    [[ "$line" == *'; mismatch_count=0;'* ]] || pass=0
    [[ "$line" == *'; nonfinite_count=0;'* ]] || pass=0
    [[ "$line" == *'; bit_exact=true;'* ]] || pass=0
    [[ "$line" == *'; worker_not_main=true;'* ]] || pass=0
    [[ "$line" == *'; work_owner=PreloadBuilder;'* ]] || pass=0
    [[ "$line" == *'; tile_commit_authority=PQS;'* ]] || pass=0
    [[ "$line" == *'; production_authority=PQS;'* ]] || pass=0
    [[ "$line" == *'; producer_switch=false;'* ]] || pass=0
    [[ "$line" == *'; db_authority=PQS;'* ]] || pass=0
    [[ "$line" == *'; exact_cpu_db_write=false;'* ]] || pass=0
    [[ "$line" == *'; worker_runtime_object_access=false'* ]] || pass=0
    [[ "$commit" == *'; pass=true;'* ]] || pass=0
    [[ "$commit" == *'; commit_target=RAM_CALLBACK_ONLY;'* ]] || pass=0
  done

  echo "=== R043 PRELOAD PTC SHADOW INTEGRATION HARVEST ==="
  echo "$identity"
  grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATED_SNAPSHOT]' "$segment" || true
  grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATED_TILE]' "$segment" | grep -F '; proof_request=true;' || true
  grep -F '[AERIS43][R043_PRELOAD_PTC_INTEGRATION_HARNESS_COMMIT]' "$segment" || true
  echo "$complete"
  rm -f "$segment"

  if [[ "$pass" -ne 1 ]]; then
    echo "R043_PRELOAD_PTC_SHADOW_INTEGRATION=FAIL"
  ...
  echo "integrated_pipeline=REAL_AERISTerrainBlockPipeline"
  echo "work_owner=PreloadBuilder"
  echo "integration_bodies=7"
  echo "integration_tiles=7"
  echo "integration_resolution=17"
  echo "integration_checks_per_body=289"
  echo "integration_total_checks=2023"
  echo "integration_bit_exact=true"
  echo "commit_target=RAM_CALLBACK_ONLY"
  ...
  echo "R043_PRELOAD_PTC_SHADOW_INTEGRATION=PASS"
  echo "AERIS_CURRENT_STAGE=PASS"
  echo "next=R043_PRELOAD_PTC_SHADOW_LIVE_PRELOAD"
  return 0
}

if [[ -f "$STATE" ]]; then
  if harvest_if_ready; then exit 0; fi
  echo "INFO: stale R043 integration state replaced for current HEAD"
  rm -rf "$STATE_DIR"
fi
...
echo
echo "=== XBUILD / R043 PRELOAD PTC SHADOW INTEGRATION ==="
...
echo
echo "=== R043 PRELOAD PTC SHADOW INTEGRATION INSTALLED ==="
echo "HEAD=$HEAD_SHA"
...
echo "expected_checks=2023"
echo "commit_target=RAM_CALLBACK_ONLY"
echo "canonical_integration_source=true"
echo "preload_builder_mutated=false"
echo "preload_database_mutated=false"
echo "terrain_tile_system_mutated=false"
echo "terrain_awareness_mutated=false"
echo "worker_scheduler=AERIS_SHARED_GENERAL_COMPUTE"
echo "production_authority=PQS"
echo "producer_switch=false"
echo "db_authority=PQS"
