...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris39_mapso3e_pipeline_isolation_diagnostic.sh <KSP root>" >&2
  exit 2
fi
...
CANDIDATE="AERIS39_MAPSO3E_PIPELINE_ISOLATION_DIAGNOSTIC_V1"
...
TMP_CSPROJ="$SRC/.AERIS39_MAPSO3E.csproj"
...
test -z "$(git status --porcelain)" || {
  echo "STOP: worktree dirty before MAPSO-3E" >&2
  git status -sb
  exit 6
}
...
write_artifacts() {
  ...
  grep '\[AERIS39\]\[MAPSO3E_' "$segment" > "$OUT/mapso3e_runtime_excerpt.txt" || true
...
diagnostic_observer_source_sha256=$(sha256sum "$SRC/Terrain/AERIS39MapSoPipelineIsolationDiagnosticObserver.cs" | awk '{print $1}')
...
EOF
  ...
  (
    cd "$OUT"
    sha256sum mapso3e_runtime_excerpt.txt provenance.txt > SHA256SUMS.txt
  )
...
harvest_if_ready() {
  ...
  segment="$(mktemp /tmp/AERIS39_MAPSO3E_segment.XXXXXX)"
  ...
  if grep -Fq "[AERIS39][MAPSO3E_FAIL]" "$segment"; then
    write_artifacts "$segment" "FAIL" "$installed_sha"
    echo "=== MAPSO-3E RUNTIME FAILURE ==="
  ...
  if ! grep -Fq "[AERIS39][MAPSO3E_COMPLETE]" "$segment"; then
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
    echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
    return 0
  ...
  local complete
  complete="$(grep -F "[AERIS39][MAPSO3E_COMPLETE]" "$segment" | tail -n 1)"

  local pass=1
  [[ "$complete" == *"; candidate=$CANDIDATE;"* ]] || pass=0
  [[ "$complete" == *"; diagnostic_complete=true;"* ]] || pass=0
  [[ "$complete" == *"; bodies=6;"* ]] || pass=0
  [[ "$complete" == *"; unity_mathf_calls_thread=MAIN_THREAD_ONLY;"* ]] || pass=0
  ...
  for body in Kerbin Eve Duna Dres Moho Eeloo; do
    grep -F "[AERIS39][MAPSO3E_BODY]" "$segment" | grep -Fq "; body=$body;" || pass=0
  done

  write_artifacts "$segment" "$([[ "$pass" -eq 1 ]] && echo PASS || echo FAIL_ACCEPTANCE)" "$installed_sha"

  if [[ "$pass" -ne 1 ]]; then
    echo "=== MAPSO-3E ACCEPTANCE FAILURE ==="
    grep '\[AERIS39\]\[MAPSO3E_' "$segment" || true
    echo "archive=$ARCHIVE"
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=FAIL"
  ...
  echo "=== MAPSO-3E PIPELINE ISOLATION ==="
  grep '\[AERIS39\]\[MAPSO3E_BODY\]' "$segment" || true
  grep '\[AERIS39\]\[MAPSO3E_DETAIL\]' "$segment" | head -n 24 || true
  echo "$complete"
  echo
  ...
  echo "AERIS39_MAPSO3E_CAPTURE=PASS"
  echo "AERIS_CURRENT_STAGE=PASS"
  echo "next=MAPSO-3E_ANALYSIS"
  return 0
}
...
if [[ -f "$STATE" ]]; then
  ...
  echo "INFO: stale MAPSO-3E state replaced for current HEAD"
  rm -rf "$STATE_DIR"
fi
...
insert = (
    ...
    "    <Compile Include=\"Terrain\\AERIS39MapSoPipelineIsolationDiagnosticObserver.cs\" />\n"
    "</ItemGroup><Import Project=\"$(MSBuildToolsPath)\\Microsoft.CSharp.targets\" /></Project>"
)
if needle not in src:
    raise SystemExit("MAPSO3E project insertion point not found")
...
(
  cd "$SRC"
  xbuild /p:Configuration=Release /p:KSPDIR="$KSP" .AERIS39_MAPSO3E.csproj
)
...
echo "AERIS39_MAPSO3E_BUILD_INSTALL=PASS"
echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
