...
PREFIX = '[AERIS29 REV3.5 SALBUTAMOL SULFATE R017 RUNTIME]'
...
parser = argparse.ArgumentParser(
    description='Reconstruct formal R014 lineage, retain R015/R016 diagnostics, overlay measurement-only R017 ND presentation-stall attribution, verify, build and install once.')
...
if has(R, R013) or 'REV3_5_R013_VARIANT=' in B.read_text():
    raise SystemExit(RED + PREFIX + ' R013 REJECTED EXPERIMENT LEAKED INTO R017 PARENT' + RESET)
...
run([sys.executable, ROOT / 'Tools/apply_aeris29_rev3_5_salbutamol_r016_fdr_high_rate_diagnostics_isolation.py'])
run([sys.executable, ROOT / 'Tools/verify_aeris29_rev3_5_salbutamol_r016_fdr_high_rate_diagnostics_isolation.py'])
...
checks = [
    ...
    (('rev3_5_r017_variant=' + R017) in ident, 'R017 identity installed'),
    ...
    (marker_in_bytes(dll, 'OH_REV3_5_R017_ND_PRESENT_STALL'), 'DLL embeds R017 stall telemetry prefix'),
    (marker_in_bytes(dll, 'operationHealthRev35R017BlockedRenderedFalse'), 'DLL embeds R017 blocker counter symbols'),
    (marker_in_bytes(dll, 'blocked_rendered_false='), 'DLL embeds R017 rendered-false record field'),
    (marker_in_bytes(dll, 'blocked_foundation_flag='), 'DLL embeds R017 foundation record field'),
    (marker_in_bytes(dll, 'blocked_coverage='), 'DLL embeds R017 coverage record field'),
    (marker_in_bytes(dll, 'blocked_ready_far='), 'DLL embeds R017 readyFar record field'),
...
print(YELLOW + 'R017 CONTRACT:' + RESET +
      ' measurement only. Existing back-render completion and cadence branches gain counters only. A persistent 10Hz observer reports START/END only when a committed FRONT is >=0.25s old and real presentation demand remains. Stationary retained FRONTs are excluded. No ND presentation, renderer, worker, AP/FBW/PROTECT, quality, 10Hz, 160km, R014 publication or R016 isolation authority is changed.')
