...
suite = CheckSuite("v0.18.0.0 Gate 0 static verification")
version_txt, version_json, version_cs = parse_version()
suite.equal(version_txt, "0.18.0.0", "VERSION is v0.18.0.0")
...
suite.check(('GUILayout.Label("RESULT "+registry.Status)' in window_code) or
            ('WrappedAirfieldLabel("RESULT  "+registry.Status)' in window_code),
...
suite.check("ProviderGeometryDiagnosticIdentity(record)" in registry_code and
            "geometryDiagnosticSignature = HashProviderIdentities(geometryIdentities)" in
            registry_code and '"; geometrySignature=" +' in registry_code,
            "provider identity and runtime geometry signatures are separately published")
contracts_code = read(SOURCE / "Landing" / "AERISRunwaySurveyContracts.cs")
snapshot_builder_code = read(SOURCE / "Landing" / "AERISRunwaySnapshotBuilder.cs")
suite.check("CurrentAlgorithmVersion = 1710" in contracts_code,
            "sub-metre compatibility cache has a new algorithm version")
for token in ("StableProviderFingerprintIdentity(record)",
    ...
    suite.check(token in snapshot_builder_code,
                "canonical source cache fingerprint contract: " + token)
fingerprint_start = snapshot_builder_code.index("static string BuildFingerprint")
fingerprint_end = snapshot_builder_code.index(
    "static string StableProviderFingerprintIdentity", fingerprint_start)
fingerprint_code = snapshot_builder_code[fingerprint_start:fingerprint_end]
for forbidden in ("BuildPrimitiveAggregate(primitives", "BuildPointAggregate(points",
...
suite.check("root.worldToLocalMatrix * child.localToWorldMatrix" not in
            snapshot_builder_code,
            "canonical source key avoids planet-scale world-matrix cancellation")
suite.check("ProviderReferencePositionValid" in snapshot_builder_code,
            "provider-authored placement is preferred over runtime fallback")
cache_code = read(SOURCE / "Landing" / "AERISRunwayCertificationCache.cs")
for token in ("CurrentSchemaVersion = 8", "PreviousSchemaVersion = 7",
              ...
              "MaximumScaleDelta = 0.0005", "SourceFingerprint = snapshot.SourceFingerprint",
              'Add(node, "sourceFingerprint", record.SourceFingerprint)'):
    suite.check(token in cache_code,
                "strict sub-metre compatibility contract: " + token)
for token in ("BuildSourceFingerprint(record",
              "SourceFingerprint = sourceFingerprint",
              "sub-metre compatible hit",
              "CERTIFIED CACHE HIT — SUB-METRE PROVIDER FRAME COMPATIBILITY"):
    suite.check(token in (snapshot_builder_code + contracts_code + registry_code),
                "source/placement split integration: " + token)

identity_start = registry_code.index("static string ProviderSignatureIdentity")
identity_end = registry_code.index("static string ProviderGeometryDiagnosticIdentity")
identity_code = registry_code[identity_start:identity_end]
for forbidden in ("LatitudeDeg", "LongitudeDeg", "ElevationMeters",
                  "OrientationHeadingDeg", "DeclaredLengthMeters",
                  "DeclaredWidthMeters", "RuntimeModelScale",
                  "QuantizedSignatureNumber"):
    suite.check(forbidden not in identity_code,
                "restart-stable provider identity excludes " + forbidden)

provider_identity = read(SOURCE / "Landing" / "AERISProviderIdentity.cs")
for token in ("internal static class AERISProviderIdentity",
              "hasStableProviderFields", "uuidFallback",
              "ComposeStableRecordId", "normalizedPath"):
    suite.check(token in provider_identity,
                "shared provider cache identity: " + token)
suite.check('Landing\\AERISProviderIdentity.cs' in includes,
            "shared provider identity helper is compiled")
suite.check("canonical identity migration compacted" in cache_code and
            "failureAliasesCompacted" in cache_code,
            "schema-6 load compacts volatile provider UUID aliases")
suite.check("NormalizeCachedStableIds" in cache_code,
            "schema migration rewrites embedded runway/direction stable IDs")
