...
ck('bool contentTickRequired = contentGeometryChanged || workerResultReady || contentRetryDue;' in RF,'content maintenance has explicit gate')
...
ck(R.count('if (contentGpuReadyPending)') >= 2 and R.count('MarkVisibleGpuReady(tiles);') == 2,'visible GPU READY scan occurs only after changed-content main-path commit')
ck('nextAuthoritativePresentationTickRealtime = presentationNow + 0.10f' in RF,'exact projection/presentation remains fixed 10 Hz')
...
ck(V.get('NAME') in approved_names,'runtime identity is Step 2 or approved Step 3 successor')
ck(('OPERATION HEALTH STEP 2 MOTION CONTENT SPLIT COASTAL EDGE REFINEMENT' in B) or
   ('OPERATION HEALTH STEP 3 WORKER PROJECTION' in B),'Ubuntu build identifies Step 2 lineage or Step 3 successor')
# Pure numerical guard for the presentation crossing contract.
