...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris41_r041_eeloo_heightnoise_vertheight_il_closure.sh <KSP root>" >&2
  exit 2
fi
...
CANDIDATE="AERIS41_R041_EELOO_HEIGHTNOISE_VERTHEIGHT_IL_CLOSURE_V1"
...
TEMPLATE="$SRC/Terrain/AERIS41EelooVoronoiIlClosureObserver.cs"
TRANSFORMER="$ROOT/Tools/aeris41_transform_voronoi_il_observer_to_heightnoise.py"
TMP_OBSERVER="$SRC/Terrain/.AERIS41EelooHeightNoiseVertHeightIlClosureObserver.cs"
TMP_CSPROJ="$SRC/.AERIS41_R041_EELOO_HEIGHTNOISE_IL.csproj"
...
OUT="$ARTIFACT_ROOT/AERIS41_R041_Eeloo_HeightNoiseVertHeight_IL_Closure"
ARCHIVE="$ARTIFACT_ROOT/AERIS41_R041_Eeloo_HeightNoiseVertHeight_IL_Closure.tar.gz"

KEY="$(printf '%s' "$KSP" | sha256sum | awk '{print substr($1,1,16)}')"
STATE_DIR="$HOME/.cache/AERIS/r041-eeloo-heightnoise-vertheight-il/$KEY"
...
[[ -f "$TEMPLATE" ]] || { echo "STOP: IL observer template missing" >&2; exit 4; }
[[ -f "$TRANSFORMER" ]] || { echo "STOP: height-noise observer transformer missing" >&2; exit 4; }
...
test "$(git branch --show-current)" = "$BRANCH" || { echo "STOP: wrong branch" >&2; git branch --show-current >&2; exit 5; }
test -z "$(git status --porcelain)" || { echo "STOP: worktree dirty before Eeloo HeightNoiseVertHeight IL closure" >&2; git status -sb >&2; exit 6; }
...
write_artifacts() {
  ...
  rm -rf "$OUT"
  mkdir -p "$OUT"
  grep -F '[AERIS41][HEIGHTNOISE_IL]' "$segment" > "$OUT/heightnoise_il_runtime_excerpt.txt" || true
...
template_source_sha256=$(sha256sum "$TEMPLATE" | awk '{print $1}')
transformer_source_sha256=$(sha256sum "$TRANSFORMER" | awk '{print $1}')
runner_source_sha256=$(sha256sum "$ROOT/Tools/aeris41_r041_eeloo_heightnoise_vertheight_il_closure.sh" | awk '{print $1}')
...
EOF
  (cd "$OUT" && sha256sum heightnoise_il_runtime_excerpt.txt provenance.txt > SHA256SUMS.txt)
  rm -f "$ARCHIVE"
  tar -C "$ARTIFACT_ROOT" -czf "$ARCHIVE" AERIS41_R041_Eeloo_HeightNoiseVertHeight_IL_Closure
}
...
harvest_if_ready() {
  ...
  segment="$(mktemp /tmp/AERIS41_R041_HEIGHTNOISE_IL_segment.XXXXXX)"
  ...
  if grep -Fq '[AERIS41][HEIGHTNOISE_IL][FAIL]' "$segment"; then
    write_artifacts "$segment" "FAIL" "$installed_sha"
    echo "=== R041 EELOO HEIGHTNOISE VERTHEIGHT IL CLOSURE RUNTIME FAILURE ==="
    grep -F '[AERIS41][HEIGHTNOISE_IL]' "$segment" || true
  ...
  if ! grep -Fq '[AERIS41][HEIGHTNOISE_IL][COMPLETE]' "$segment"; then
    local begin_seen=false
    grep -Fq '[AERIS41][HEIGHTNOISE_IL][BEGIN]' "$segment" && begin_seen=true
    rm -f "$segment"
  ...
  complete="$(grep -F '[AERIS41][HEIGHTNOISE_IL][COMPLETE]' "$segment" | tail -n 1)"
  [[ "$complete" == *"; pass=true;"* ]] || pass=0
  [[ "$complete" == *"; candidate=$CANDIDATE;"* ]] || pass=0
  [[ "$complete" == *"; body=Eeloo;"* ]] || pass=0
  [[ "$complete" == *"; target_instances="* ]] || pass=0
  ...
  local target_line target_method field_line
  target_line="$(grep -F '[AERIS41][HEIGHTNOISE_IL][TARGET]' "$segment" | grep -F '; type=PQSMod_VertexHeightNoiseVertHeight;' | head -n 1 || true)"
  target_method="$(grep -F '[AERIS41][HEIGHTNOISE_IL][METHOD]' "$segment" | grep -F '; role=TARGET;' | grep -F '; method=OnVertexBuildHeight(' | head -n 1 || true)"
  field_line="$(grep -F '[AERIS41][HEIGHTNOISE_IL][FIELD]' "$segment" | head -n 1 || true)"
  [[ -n "$target_line" ]] || pass=0
  [[ -n "$target_method" ]] || pass=0
  [[ -n "$field_line" ]] || pass=0

  if [[ "$pass" -ne 1 ]]; then
    write_artifacts "$segment" "FAIL_ACCEPTANCE" "$installed_sha"
    echo "=== R041 EELOO HEIGHTNOISE VERTHEIGHT IL CLOSURE ACCEPTANCE FAILURE ==="
    grep -F '[AERIS41][HEIGHTNOISE_IL]' "$segment" || true
    echo "archive=$ARCHIVE"
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=FAIL"
  ...
  echo "=== R041 EELOO HEIGHTNOISE VERTHEIGHT IL CLOSURE ACCEPTANCE ==="
  grep -F '[AERIS41][HEIGHTNOISE_IL][TARGET]' "$segment" || true
  grep -F '[AERIS41][HEIGHTNOISE_IL][FIELD]' "$segment" || true
  grep -F '[AERIS41][HEIGHTNOISE_IL][DEPENDENCY_INSTANCE]' "$segment" || true
  grep -F '[AERIS41][HEIGHTNOISE_IL][DEPENDENCY_TYPE]' "$segment" || true
  grep -F '[AERIS41][HEIGHTNOISE_IL][METHOD]' "$segment" || true
  grep -F '[AERIS41][HEIGHTNOISE_IL][LOCAL]' "$segment" || true
  grep -F '[AERIS41][HEIGHTNOISE_IL][INSN]' "$segment" || true
  echo "$complete"
  echo
  ...
  rm -f "$segment"
  rm -rf "$STATE_DIR"
  echo "AERIS41_R041_EELOO_HEIGHTNOISE_VERTHEIGHT_IL_CLOSURE=PASS"
  echo "AERIS_CURRENT_STAGE=PASS"
  echo "next=R041_EELOO_HEIGHTNOISE_VERTHEIGHT_PURE_CPU_EXACT"
  return 0
}

if [[ -f "$STATE" ]]; then
  if harvest_if_ready; then exit 0; fi
  echo "INFO: stale Eeloo HeightNoiseVertHeight IL closure state replaced for current HEAD"
  rm -rf "$STATE_DIR"
fi
...
echo "=== R041 EELOO HEIGHTNOISE VERTHEIGHT IL CLOSURE BUILD ==="
echo "HEAD=$HEAD"
echo "KSP=$KSP"
...
python3 "$TRANSFORMER" "$TEMPLATE" "$TMP_OBSERVER"

grep -Fq 'PQSMod_VertexHeightNoiseVertHeight' "$TMP_OBSERVER" || { echo "STOP: transformed observer target missing" >&2; exit 18; }
grep -Fq '[AERIS41][HEIGHTNOISE_IL][COMPLETE]' "$TMP_OBSERVER" || { echo "STOP: transformed observer log prefix missing" >&2; exit 19; }

python3 - "$CSPROJ" "$TMP_CSPROJ" <<'PY'
import pathlib, sys
src = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8")
needle = '</ItemGroup><Import Project="$(MSBuildToolsPath)\\Microsoft.CSharp.targets" /></Project>'
include = '    <Compile Include="Terrain\\.AERIS41EelooHeightNoiseVertHeightIlClosureObserver.cs" />\n'
if src.count(needle) != 1:
    raise SystemExit("AERIS41 HeightNoise project insertion point not unique")
...
(cd "$SRC" && xbuild /p:Configuration=Release /p:KSPDIR="$KSP" .AERIS41_R041_EELOO_HEIGHTNOISE_IL.csproj)
...
echo
echo "=== R041 EELOO HEIGHTNOISE VERTHEIGHT IL CLOSURE INSTALLED ==="
echo "dll_sha256=$INSTALLED_SHA"
echo "previous_dll_sha256=$OLD_SHA"
echo "backup=$BACKUP"
echo "log_offset=$LOG_OFFSET"
echo "closure=TARGET_DECLARED_IL_PLUS_RUNTIME_LIBNOISE_FIELD_GRAPH"
...
echo "AERIS41_R041_EELOO_HEIGHTNOISE_VERTHEIGHT_IL_CLOSURE_BUILD_INSTALL=PASS"
echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
