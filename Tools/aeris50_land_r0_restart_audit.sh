...
regex_absent "$LAND" 'FlightCtrlState[[:space:]]+(state|ctrl|fcs)([^A-Za-z_]|$)' "LAND foundation has no FlightCtrlState writer"
...
regex_absent "$ND" 'FlightCtrlState[[:space:]]+(state|ctrl|fcs)([^A-Za-z_]|$)' "ND has no FlightCtrlState writer"
...
regex_absent "$APP_PLAN" 'FlightCtrlState[[:space:]]+(state|ctrl|fcs)([^A-Za-z_]|$)' "Adaptive Approach planner remains control-free"
