...
PREFIX='[AERIS32 R035 LANDCONTROL WRITE SEMANTICS IL AUDIT BUILD]'
...
parent=ROOT/'Source/AERISFlightControl/Terrain/AERISR034PqsLandControlHeightPathAuditObserver.cs'
...
if not parent.is_file() or PARENT not in version.read_text() or 'AERISR034PqsLandControlHeightPathAuditObserver.cs' not in csproj.read_text():
    raise SystemExit(PREFIX+' accepted R034 materialization missing. Do NOT reset/clean/stash. Materialize/build accepted R034 first, then return to R035.')
print(PREFIX+' PASS accepted R034 materialization reused')
frozen=[
...
run([sys.executable,ROOT/'Tools/apply_aeris32_rev3_5_r035_landcontrol_write_semantics_il_audit.py'])
run([sys.executable,ROOT/'Tools/verify_aeris32_rev3_5_r035_landcontrol_write_semantics_il_audit.py'])
for p in frozen:
...
for token in (MARKER,'[R035][LANDCTRL_IL] event=WRITE_SEMANTICS_IL_AUDIT_COMPLETE','[R035][LANDCTRL_IL_INSN]','worker_dispatch=false','worker_invokes_runtime_object=false','authority=PQS'):
...
+'source_head='+head+'\n'
+'parent='+PARENT+'\n'
+'runtime_change=LANDCONTROL_WRITE_SEMANTICS_IL_AUDIT_SHADOW_ONLY\n'
...
print('TEST: restart KSP to Main Menu and wait for [R035][LANDCTRL_IL] event=WRITE_SEMANTICS_IL_AUDIT_COMPLETE')
