...
check('internal double OceanFloorMeters;' in s,'relative ocean floor scalar in snapshot')
check('ReadDouble(mod,"oceanRad")' in s and 'ReadDouble(sphere,"radius")' in s and 'oceanRad-sphereRadius' in s,
      'absolute ocean radius converted to relative worker height')
check('[R037][FLATTEN]' in s and 'relative_floor_m=' in s,'FlattenOcean scalar telemetry')
check(FLATTEN_IL_SHA in s,'FlattenOcean accepted IL hash guard')
check('return h<st.OceanFloorMeters?st.OceanFloorMeters:h;' in s,'FlattenOcean exact relative max semantics')
