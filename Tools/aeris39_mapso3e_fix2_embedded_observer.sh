...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris39_mapso3e_fix2_embedded_observer.sh <KSP root>" >&2
  exit 2
fi

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
KSP="$1"
PATCH="$ROOT/Tools/AERIS39_MAPSO3E_fix2_embedded_observer_patch.py"
...
OBSERVER_OUT="/tmp/AERIS39MapSoPipelineIsolationDiagnosticObserver.Fix2.cs"
RUNNER_OUT="/tmp/aeris39_mapso3e_fix2_pipeline_isolation_diagnostic.sh"
...
[[ -f "$PATCH" ]] || { echo "STOP: MAPSO-3E Fix2 patch missing" >&2; exit 20; }
