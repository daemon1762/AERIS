...
suite = CheckSuite("v0.17.0.8 failure-hint identity compaction regression")
...
for token in ("CurrentSchemaVersion = 6", "PreviousSchemaVersion = 5",
              "CompatibilitySchemaVersion = 4", "LegacySchemaVersion = 3",
    ...
    suite.check(token in cache, "schema-6 compaction contract: " + token)
suite.check("CurrentAlgorithmVersion = 1670" in contracts,
            "identity-only schema migration preserves the validated survey algorithm")
suite.finish()
