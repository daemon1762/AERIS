...
suite = CheckSuite("v0.18.0.0 CP3 Gate 4A Terrain Supply successor")
...
suite.check(system.find("ReconcilePlannedRequests();") <
            system.find("EnsureRequest(acceptedRequestScratch[i])"),
...
suite.check(system.find("AERISRunwayDirectionDefinition direction") <
            system.find("predictiveCorridor.Build"),
            "selected-runway LAND candidates are reserved before predictive look-ahead expansion")
suite.check("MaximumPointCount = 18" in (ROOT / "Source/AERISFlightControl/Terrain/AERISPredictiveForwardCorridor.cs").read_text(),
            "Gate 3 predictive expansion remains explicitly bounded")
...
for token in (
    ...
    suite.check(token in renderer, "Gate 4A GPU-only composition: " + token)
suite.check("GUI.DrawTextureWithTexCoords(plot, backTarget" not in renderer,
            "incomplete BACK terrain is never presented")
suite.check("TERRAIN GPU BUILDING " in nd,
            "partial GPU readiness is visible without CPU terrain substitution")
