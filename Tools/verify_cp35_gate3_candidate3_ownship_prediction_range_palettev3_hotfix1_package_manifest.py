...
suite=CheckSuite('CP3.5 Gate 3 Candidate 3 Authority/Palette V3 Hotfix 1 package manifest verification')
...
actual=sorted(str(p.relative_to(ROOT)).replace('\\','/') for p in ROOT.rglob('*') if p.is_file() and p.name!='MANIFEST_SHA256.txt' and '__pycache__' not in p.parts and p.suffix.lower()!='.pyc')
