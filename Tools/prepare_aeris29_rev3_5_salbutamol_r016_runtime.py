...
PREFIX = '[AERIS29 REV3.5 SALBUTAMOL SULFATE R016 RUNTIME]'
...
parser = argparse.ArgumentParser(
    description='Reconstruct formal R014, retain R015 Full-GC observer, overlay R016 high-rate built-in FDR diagnostic isolation, verify, build and install once.')
...
if has(R, R013) or 'REV3_5_R013_VARIANT=' in B.read_text():
    raise SystemExit(RED + PREFIX + ' R013 REJECTED EXPERIMENT LEAKED INTO R016 PARENT' + RESET)
...
checks = [
    ...
    (('rev3_5_r016_variant=' + R016) in ident, 'R016 identity installed'),
    ...
    (marker_in_bytes(dll, 'OH_REV3_5_R015_GC_ATTR'), 'DLL embeds R015 GC telemetry prefix'),
    (marker_in_bytes(dll, R016), 'DLL embeds R016 isolation marker'),
    (marker_in_bytes(dll, 'R016HighRateDiagnosticsEnabled'), 'DLL embeds R016 diagnostic gate symbol'),
...
print(YELLOW + 'R016 CONTRACT:' + RESET +
      ' A/B isolation only. Nine built-in control-cadence FDR diagnostic producers return before BeginFlight/CSV capture, removing their continuous row allocation/enqueue. Writer creation and headers remain. Core 10Hz FDR, CVR, extension telemetry, AA comparison, R015 Full-GC observer, R014 publication batching, ND/AP/FBW/PROTECT behavior, fixed 10Hz/160km and Golden painter order remain unchanged.')
