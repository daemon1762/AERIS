...
PREFIX = '[AERIS28 REV3.5 SALBUTAMOL SULFATE R013 RUNTIME]'
...
parser = argparse.ArgumentParser(
    description='Reconstruct R010 formal runtime, retain R011/R012, overlay R013 stable content snapshot reconcile, verify, build and install.')
...
# Verify the performance parent before layering the observer and bugfix successors.
for verifier in (
...
run([sys.executable, ROOT / 'Tools/apply_aeris28_rev3_5_salbutamol_r013_stable_content_snapshot_reconcile.py'])
run([sys.executable, ROOT / 'Tools/verify_aeris28_rev3_5_salbutamol_r013_stable_content_snapshot_reconcile.py'])
run(['git', 'diff', '--check'])
...
# build_ubuntu.sh now contains the complete inherited verifier/prebuild chain plus R013.
...
checks = [
    ...
    (('rev3_5_r013_variant=' + R013) in ident, 'R013 identity installed'),
    ...
    (marker_in_bytes(dll, R013), 'DLL embeds R013 variant marker'),
    (marker_in_bytes(dll, 'oh_rev35_r013_snapshot_reuse='), 'DLL embeds R013 snapshot-reuse telemetry'),
    (marker_in_bytes(dll, 'oh_rev35_r013_full_capture='), 'DLL embeds R013 true-capture telemetry'),
...
print(YELLOW + 'R013 CONTRACT:' + RESET +
      ' true geometry/view refresh owns CaptureVisible and planner anchors; completion/retry maintenance reuses the stable content material snapshot while existing Entry resolve/foundation/schedule progress continues. REV009 cumulative 6deg planning, R010 single commit lane, fixed 10Hz/160km, complete coverage, Golden painter order and R012 fixes remain unchanged.')
