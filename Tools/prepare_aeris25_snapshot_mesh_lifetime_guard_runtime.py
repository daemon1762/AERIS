...
def rev008_ready():
    try:
        ...
        return False
    return ('internal const string Revision = "OH_PHASE4_008";' in m and
            'AERIS25_SNAPSHOT_MESH_LIFETIME_GUARD' in r and
            'oh_snapshot_stale_mesh=' in r and
            'REV008 SNAPSHOT MESH LIFETIME GUARD' in u and
            'verify_aeris25_snapshot_mesh_lifetime_guard_hotfix.py' in u)


parser = argparse.ArgumentParser(description='Prepare/install AERIS25 ATROPINE rev008 Snapshot Mesh Lifetime Guard.')
...
if not ksp.is_dir():
    raise SystemExit('[AERIS25 REV008 RUNTIME] KSP path not found: ' + str(ksp))

if not rev008_ready():
    # Reconstruct the exact tested lineage. rev005 and rev006 are retained as historical
    # generation steps; rev006 rolls the rejected rev005 behaviour back before rev007/008.
    steps = [
    ...
    print('[AERIS25 REV008 RUNTIME] generated rev008 tree already present; reconstruction skipped')
...
if (ksp / 'KSP_x64_Data/Managed/Assembly-CSharp.dll').is_file():
    ...
    raise SystemExit('[AERIS25 REV008 RUNTIME] could not identify KSP Unity player layout')
...
if need_rebuild:
    ...
    print('[AERIS25 REV008 RUNTIME] no shader change; reusing accepted AERIS25 bundle/probe pair')

for path, label in ((bundle, 'shader'), (probe, 'probe')):
    if not path.is_file() or path.stat().st_size == 0 or not assetbundle_ok(path):
        raise SystemExit('[AERIS25 REV008 RUNTIME] invalid %s bundle: %s' % (label, path))
if shader_mode == 'windows' and sha256(probe) != EXPECTED_WINDOWS_PROBE_SHA:
    raise SystemExit('[AERIS25 REV008 RUNTIME] Windows probe compatibility SHA FAIL')
...
for path in (source_dll, installed_dll, installed_bundle, installed_probe, identity, config):
    if not path.is_file():
        raise SystemExit('[AERIS25 REV008 RUNTIME] installed artifact missing: ' + str(path))
...
for ok, label in checks:
    ...
    if not ok:
        failed.append(label)
if failed:
    raise SystemExit('[AERIS25 REV008 RUNTIME] INSTALL IDENTITY FAIL: ' + ', '.join(failed))

print('[AERIS25 REV008 RUNTIME] INSTALL IDENTITY MATCH=YES')
...
print('Runtime gate: require GPU_ACTIVE + Dynamic Colour ACTIVE + oh_snapshot_stale_mesh=0.')
print('Root-cause gate: oh_gpu_vertex_reject_semantic_mesh_null must stop increasing from snapshot pruning.')
print('Golden gate: zero rectangular holes, especially 160 km Track-Up ocean/edge regions.')
