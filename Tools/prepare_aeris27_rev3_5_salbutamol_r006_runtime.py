...
R006_HF2 = 'AERIS27_REV3_5_SALBUTAMOL_SULFATE_R006_RESOURCE_RELEASE_ORDER_HOTFIX2'
...
parser = argparse.ArgumentParser(
    description='Prepare/install AERIS27 REV3.5 SALBUTAMOL SULFATE R006 + Resource Release Hotfix1/2. R006 keeps R005/R004 behavior, reuses only snapshot-safe retired bare GeographicUnitPoint arrays, transfers already-complete source Vector3 arrays into CPU fallback projected ownership, and adds measurement-only foundation/finalize/GPU-scratch observers. HF1/HF2 ensure full terrain OFF/suspend/dispose drains both native and managed recycle pools even after ResetContentSnapshot teardown. No foundation admission or publication semantics are changed.')
...
if not marker_present(R006_HF2):
    run([sys.executable,
...
checks = [
    ...
    (('rev3_5_r006_hotfix2=' + R006_HF2) in identity_text,
     'R006 resource-release-order Hotfix2 identity marker'),
    ...
    (marker_in_bytes(dll, R006_HF2), 'DLL embeds R006 Hotfix2 marker'),
...
print(YELLOW + 'R006 RESOURCE RELEASE:' + RESET + ' ordinary Entry retirement may reuse bounded bare arrays; full teardown drains again after ResetContentSnapshot so pending/deferred teardown cannot repopulate native/managed pools.')
