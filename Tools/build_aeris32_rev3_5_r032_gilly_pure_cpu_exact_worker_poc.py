...
PREFIX='[AERIS32 R032 GILLY PURE CPU WORKER BUILD]'
...
parent=ROOT/'Source/AERISFlightControl/Terrain/AERISR031PtcGillyDependencyClosureObserver.cs';csproj=ROOT/'Source/AERISFlightControl/AERISFlightControl.csproj'
if not parent.is_file() or PARENT not in parent.read_text() or 'AERISR031PtcGillyDependencyClosureObserver.cs' not in csproj.read_text():
 raise SystemExit(PREFIX+' accepted R031 materialization missing. Do NOT reset/clean. Switch back to the accepted R031 branch, materialize/build Hotfix1 once, then return to R032.')
print(PREFIX+' PASS accepted R031 materialization reused')
...
run([sys.executable,ROOT/'Tools/apply_aeris32_rev3_5_r032_gilly_pure_cpu_exact_worker_poc.py'])
run([sys.executable,ROOT/'Tools/verify_aeris32_rev3_5_r032_gilly_pure_cpu_exact_worker_poc.py'])
for p in frozen:
...
for token in (MARKER,'[R032][PTC_GILLY_CPU] event=PURE_CPU_EXACT_WORKER_COMPLETE','worker_invokes_runtime_object=false','snapshot_payload=PRIMITIVES_ONLY','authority=PQS'):
 if not marker(data,token):raise SystemExit(PREFIX+' DLL missing '+token)
head=out(['git','rev-parse','HEAD']);identity=('aeris_candidate='+MARKER+'\nbranch='+BRANCH+'\nsource_head='+head+'\nparent='+PARENT+'\nruntime_change=PTC_GILLY_PURE_CPU_EXACT_WORKER_SHADOW_ONLY\nworker_snapshot=PRIMITIVES_ONLY\nworker_invokes_runtime_object=NO\nterrain_db_write=NO\nproducer_switch=NO\nterrain_authority=PQS\ngpu=NO\ncertification=NO_SHADOW_ONLY\nplugin_data_before_files='+str(bfiles)+'\nplugin_data_before_bytes='+str(bbytes)+'\nplugin_data_after_files='+str(afiles)+'\nplugin_data_after_bytes='+str(abytes)+'\ndll_sha256='+sha(dll)+'\n')
(repo/'AERISCandidateBuildIdentity.txt').write_text(identity);(installed/'AERISCandidateBuildIdentity.txt').write_text(identity)
print(PREFIX+' PASS');print('head='+head);print('plugin_data_files='+str(afiles));print('plugin_data_bytes='+str(abytes));print('dll_sha256='+sha(dll));print('TEST: restart KSP to Main Menu and wait for [R032][PTC_GILLY_CPU] event=PURE_CPU_EXACT_WORKER_COMPLETE')
