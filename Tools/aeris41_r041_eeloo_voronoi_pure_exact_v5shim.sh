...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris41_r041_eeloo_voronoi_pure_exact_v5shim.sh <KSP root>" >&2
  exit 2
fi
...
TERRAINALTITUDE_EXACT_PUBLIC_PQS_INJECTOR="$ROOT/Tools/aeris41_run_fixed_terrainaltitude_exact_public_pqs_injector.py"
...
TERRAINALTITUDE_EXACT_PUBLIC_PQS_FIX2_INJECTOR="$ROOT/Tools/aeris41_inject_terrainaltitude_exact_public_pqs_fix2_runner_into_generated.py"
...
[[ -f "$TERRAINALTITUDE_EXACT_PUBLIC_PQS_INJECTOR" ]] || { echo "STOP: TerrainAltitude exact public-PQS shim missing" >&2; exit 20; }
...
[[ -f "$TERRAINALTITUDE_EXACT_PUBLIC_PQS_FIX2_INJECTOR" ]] || { echo "STOP: TerrainAltitude exact public-PQS fix2 injector missing" >&2; exit 22; }
[[ -f "$STATE_IDENTITY_INJECTOR" ]] || { echo "STOP: runtime state-identity injector missing" >&2; exit 23; }
[[ -f "$VORONOI_PURE" ]] || { echo "STOP: VertexVoronoi pure source missing" >&2; exit 24; }
[[ -f "$HEIGHTNOISE_PURE" ]] || { echo "STOP: VertexHeightNoiseVertHeight pure source missing" >&2; exit 25; }
...
grep -Fq 'pqs_public_global_pass=true' "$TERRAINALTITUDE_EXACT_PUBLIC_PQS_FIX2_INJECTOR" || {
  echo "STOP: TerrainAltitude V5 fix2 runner gate missing" >&2
  exit 39
}
...
inject = (
    ...
    '"$SHADOW_OBSERVER" "$SHADOW_RUNNER"\n'
    'python3 "$ROOT/Tools/aeris41_inject_terrainaltitude_argument_order_diagnostic_into_generated.py" '
    '"$SHADOW_OBSERVER" "$SHADOW_RUNNER"\n'
    'python3 "$ROOT/Tools/aeris41_run_fixed_terrainaltitude_exact_public_pqs_injector.py" '
    '"$SHADOW_OBSERVER" "$SHADOW_RUNNER"\n'
    'python3 "$ROOT/Tools/aeris41_inject_terrainaltitude_exact_public_pqs_fix1_into_generated.py" '
    '"$SHADOW_OBSERVER" "$SHADOW_RUNNER"\n'
    'python3 "$ROOT/Tools/aeris41_inject_terrainaltitude_exact_public_pqs_fix2_runner_into_generated.py" '
    '"$SHADOW_OBSERVER" "$SHADOW_RUNNER"\n'
...
for token in (
    ...
    'aeris41_inject_terrainaltitude_exact_public_pqs_fix2_runner_into_generated.py',
