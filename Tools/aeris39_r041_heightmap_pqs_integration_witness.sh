...
if [[ $# -ne 1 ]]; then
  echo "usage: bash Tools/aeris39_r041_heightmap_pqs_integration_witness.sh <KSP root>" >&2
  exit 2
fi
...
CANDIDATE="AERIS39_R041_HEIGHTMAP_PQS_INTEGRATION_WITNESS_V1"
...
TMP_CSPROJ="$SRC/.AERIS39_R041_HEIGHTMAP_PQS.csproj"
...
OUT="$ARTIFACT_ROOT/AERIS39_R041_HeightMap_PQS_Integration_Witness"
ARCHIVE="$ARTIFACT_ROOT/AERIS39_R041_HeightMap_PQS_Integration_Witness.tar.gz"

KEY="$(printf '%s' "$KSP" | sha256sum | awk '{print substr($1,1,16)}')"
STATE_DIR="$HOME/.cache/AERIS/r041-heightmap-pqs-integration/$KEY"
...
test -z "$(git status --porcelain)" || {
  echo "STOP: worktree dirty before R041 HeightMap PQS integration witness" >&2
  git status -sb
  exit 6
}
...
write_artifacts() {
  ...
  rm -rf "$OUT"
  mkdir -p "$OUT"
  grep '\[AERIS39\]\[HEIGHTMAP_PQS_' "$segment" > "$OUT/heightmap_pqs_runtime_excerpt.txt" || true
...
observer_source_sha256=$(sha256sum "$SRC/Terrain/AERIS39HeightMapPqsIntegrationWitnessObserver.cs" | awk '{print $1}')
reference=REAL_PQSMOD_VERTEXHEIGHTMAP_CALLBACK
...
EOF
  (cd "$OUT" && sha256sum heightmap_pqs_runtime_excerpt.txt provenance.txt > SHA256SUMS.txt)
  rm -f "$ARCHIVE"
  tar -C "$ARTIFACT_ROOT" -czf "$ARCHIVE" AERIS39_R041_HeightMap_PQS_Integration_Witness
}
...
harvest_if_ready() {
  ...
  segment="$(mktemp /tmp/AERIS39_R041_HEIGHTMAP_PQS_segment.XXXXXX)"
  ...
  if grep -Fq "[AERIS39][HEIGHTMAP_PQS_FAIL]" "$segment"; then
    write_artifacts "$segment" "FAIL" "$installed_sha"
    echo "=== R041 HEIGHTMAP PQS INTEGRATION RUNTIME FAILURE ==="
    grep '\[AERIS39\]\[HEIGHTMAP_PQS_' "$segment" || true
  ...
  if ! grep -Fq "[AERIS39][HEIGHTMAP_PQS_COMPLETE]" "$segment"; then
    local begin_seen=false
    grep -Fq "[AERIS39][HEIGHTMAP_PQS_BEGIN]" "$segment" && begin_seen=true
    rm -f "$segment"
  ...
  complete="$(grep -F "[AERIS39][HEIGHTMAP_PQS_COMPLETE]" "$segment" | tail -n 1)"
  ...
  [[ "$complete" == *"; reference=REAL_PQSMOD_VERTEXHEIGHTMAP_CALLBACK;"* ]] || pass=0
  ...
  for body in Kerbin Eve Duna Dres Moho Eeloo; do
    local line
    line="$(grep -F "[AERIS39][HEIGHTMAP_PQS_BODY]" "$segment" | grep -F "; body=$body;" | tail -n 1 || true)"
  ...
  if [[ "$pass" -ne 1 ]]; then
    echo "=== R041 HEIGHTMAP PQS INTEGRATION ACCEPTANCE FAILURE ==="
    grep '\[AERIS39\]\[HEIGHTMAP_PQS_' "$segment" || true
    echo "archive=$ARCHIVE"
    rm -f "$segment"
    echo "AERIS_CURRENT_STAGE=FAIL"
  ...
  echo "=== R041 HEIGHTMAP PQS INTEGRATION ACCEPTANCE ==="
  grep '\[AERIS39\]\[HEIGHTMAP_PQS_SNAPSHOT\]' "$segment" || true
  grep '\[AERIS39\]\[HEIGHTMAP_PQS_BODY\]' "$segment" || true
  echo "$complete"
  echo
  ...
  rm -f "$segment"
  rm -rf "$STATE_DIR"
  echo "AERIS39_R041_HEIGHTMAP_PQS_INTEGRATION_WITNESS=PASS"
  echo "AERIS_CURRENT_STAGE=PASS"
  echo "next=R041_ALLBODY_HEIGHT_MODIFIER_CHAIN_SHADOW"
  return 0
}

if [[ -f "$STATE" ]]; then
  if harvest_if_ready; then exit 0; fi
  echo "INFO: stale R041 HeightMap PQS integration state replaced for current HEAD"
  rm -rf "$STATE_DIR"
fi
...
echo "=== R041 HEIGHTMAP PQS INTEGRATION BUILD ==="
echo "HEAD=$HEAD"
echo "KSP=$KSP"
...
insert = (
    ...
    "    <Compile Include=\"Terrain\\AERIS39HeightMapPqsIntegrationWitnessObserver.cs\" />\n"
    "</ItemGroup><Import Project=\"$(MSBuildToolsPath)\\Microsoft.CSharp.targets\" /></Project>"
)
if needle not in src:
    raise SystemExit("R041 HeightMap PQS project insertion point not found")
pathlib.Path(sys.argv[2]).write_text(src.replace(needle, insert, 1), encoding="utf-8")
PY

rm -rf "$SRC/bin/Release" "$SRC/obj/Release"
(cd "$SRC" && xbuild /p:Configuration=Release /p:KSPDIR="$KSP" .AERIS39_R041_HEIGHTMAP_PQS.csproj)
...
echo
echo "=== R041 HEIGHTMAP PQS INTEGRATION INSTALLED ==="
...
echo "reference=REAL_PQSMOD_VERTEXHEIGHTMAP_CALLBACK"
...
echo "AERIS39_R041_HEIGHTMAP_PQS_INTEGRATION_BUILD_INSTALL=PASS"
echo "AERIS_CURRENT_STAGE=WAITING_FOR_KSP"
echo "human_action=Launch KSP to Main Menu, exit KSP, then run the same command again."
